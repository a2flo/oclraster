...
float4 FUNC_OVERLOAD OCLRASTER_FUNC image_read_float_nearest(global const uchar* img, const uint offset) {
...
float4 FUNC_OVERLOAD OCLRASTER_FUNC image_read_float_nearest(global const uchar* img, const float2 coord) {
...
float4 FUNC_OVERLOAD OCLRASTER_FUNC image_read_float_nearest(global const uchar* img, const uint2 coord) {
 const uint2 img_size = oclr_get_image_size((image_header_ptr)img);
 return image_read_float_nearest(img, coord.y * img_size.x + coord.x);
}
float4 FUNC_OVERLOAD OCLRASTER_FUNC image_read_float_linear(global const uchar* img, const float2 coord) {
 const uint2 img_size = oclr_get_image_size((image_header_ptr)img);
 global const uchar* img_data_ptr = (global const uchar*)((global const uchar*)img + OCLRASTER_IMAGE_HEADER_SIZE);
 const float2 fimg_size = convert_float2(img_size) - 1.0f;
 const float2 norm_coord = fmod(coord + fabs(floor(coord)), (float2)(1.0f, 1.0f));
...
void FUNC_OVERLOAD OCLRASTER_FUNC image_write(global uchar* img, const uint2 coord, const float4 color) {
 const uint2 img_size = oclr_get_image_size((image_header_ptr)img);
...
float4 FUNC_OVERLOAD OCLRASTER_FUNC image_read_float_nearest(global const uchar2* img, const uint offset) {
...
float4 FUNC_OVERLOAD OCLRASTER_FUNC image_read_float_nearest(global const uchar2* img, const float2 coord) {
 const uint2 img_size = oclr_get_image_size((image_header_ptr)img);
 const float2 fimg_size = convert_float2(img_size) - 1.0f;
 const float2 norm_coord = fmod(coord + fabs(floor(coord)), (float2)(1.0f, 1.0f));
 const uint2 ui_tc = clamp(convert_uint2(norm_coord * fimg_size), (uint2)(0u, 0u), img_size - 1u);
...
float4 FUNC_OVERLOAD OCLRASTER_FUNC image_read_float_nearest(global const uchar2* img, const uint2 coord) {
 const uint2 img_size = oclr_get_image_size((image_header_ptr)img);
 return image_read_float_nearest(img, coord.y * img_size.x + coord.x);
}
float4 FUNC_OVERLOAD OCLRASTER_FUNC image_read_float_linear(global const uchar2* img, const float2 coord) {
 const uint2 img_size = oclr_get_image_size((image_header_ptr)img);
 global const uchar2* img_data_ptr = (global const uchar2*)((global const uchar*)img + OCLRASTER_IMAGE_HEADER_SIZE);
 const float2 fimg_size = convert_float2(img_size) - 1.0f;
 const float2 norm_coord = fmod(coord + fabs(floor(coord)), (float2)(1.0f, 1.0f));
...
void FUNC_OVERLOAD OCLRASTER_FUNC image_write(global uchar2* img, const uint2 coord, const float4 color) {
 const uint2 img_size = oclr_get_image_size((image_header_ptr)img);
 const uint offset = coord.y * img_size.x + coord.x;
...
float4 FUNC_OVERLOAD OCLRASTER_FUNC image_read_float_nearest(global const uchar3* img, const uint offset) {
...
float4 FUNC_OVERLOAD OCLRASTER_FUNC image_read_float_nearest(global const uchar3* img, const float2 coord) {
 const uint2 img_size = oclr_get_image_size((image_header_ptr)img);
 const float2 fimg_size = convert_float2(img_size) - 1.0f;
 const float2 norm_coord = fmod(coord + fabs(floor(coord)), (float2)(1.0f, 1.0f));
 const uint2 ui_tc = clamp(convert_uint2(norm_coord * fimg_size), (uint2)(0u, 0u), img_size - 1u);
...
float4 FUNC_OVERLOAD OCLRASTER_FUNC image_read_float_nearest(global const uchar3* img, const uint2 coord) {
 const uint2 img_size = oclr_get_image_size((image_header_ptr)img);
 return image_read_float_nearest(img, coord.y * img_size.x + coord.x);
}
float4 FUNC_OVERLOAD OCLRASTER_FUNC image_read_float_linear(global const uchar3* img, const float2 coord) {
 const uint2 img_size = oclr_get_image_size((image_header_ptr)img);
 global const uchar3* img_data_ptr = (global const uchar3*)((global const uchar*)img + OCLRASTER_IMAGE_HEADER_SIZE);
 const float2 fimg_size = convert_float2(img_size) - 1.0f;
 const float2 norm_coord = fmod(coord + fabs(floor(coord)), (float2)(1.0f, 1.0f));
...
void FUNC_OVERLOAD OCLRASTER_FUNC image_write(global uchar3* img, const uint2 coord, const float4 color) {
 const uint2 img_size = oclr_get_image_size((image_header_ptr)img);
 const uint offset = coord.y * img_size.x + coord.x;
...
float4 FUNC_OVERLOAD OCLRASTER_FUNC image_read_float_nearest(global const uchar4* img, const uint offset) {
...
float4 FUNC_OVERLOAD OCLRASTER_FUNC image_read_float_nearest(global const uchar4* img, const float2 coord) {
 const uint2 img_size = oclr_get_image_size((image_header_ptr)img);
 const float2 fimg_size = convert_float2(img_size) - 1.0f;
 const float2 norm_coord = fmod(coord + fabs(floor(coord)), (float2)(1.0f, 1.0f));
 const uint2 ui_tc = clamp(convert_uint2(norm_coord * fimg_size), (uint2)(0u, 0u), img_size - 1u);
...
float4 FUNC_OVERLOAD OCLRASTER_FUNC image_read_float_nearest(global const uchar4* img, const uint2 coord) {
 const uint2 img_size = oclr_get_image_size((image_header_ptr)img);
 return image_read_float_nearest(img, coord.y * img_size.x + coord.x);
}
float4 FUNC_OVERLOAD OCLRASTER_FUNC image_read_float_linear(global const uchar4* img, const float2 coord) {
 const uint2 img_size = oclr_get_image_size((image_header_ptr)img);
 global const uchar4* img_data_ptr = (global const uchar4*)((global const uchar*)img + OCLRASTER_IMAGE_HEADER_SIZE);
 const float2 fimg_size = convert_float2(img_size) - 1.0f;
 const float2 norm_coord = fmod(coord + fabs(floor(coord)), (float2)(1.0f, 1.0f));
...
void FUNC_OVERLOAD OCLRASTER_FUNC image_write(global uchar4* img, const uint2 coord, const float4 color) {
 const uint2 img_size = oclr_get_image_size((image_header_ptr)img);
 const uint offset = coord.y * img_size.x + coord.x;
...
#if defined(OCLRASTER_IMAGE_UCHAR)
uint4 FUNC_OVERLOAD OCLRASTER_FUNC image_read_uint_nearest(global const uchar* img, const uint offset) {
 global const uchar* img_data_ptr = (global const uchar*)((global const uchar*)img + OCLRASTER_IMAGE_HEADER_SIZE);
 const uchar texel = img_data_ptr[offset];
 return (uint4)( ((convert_uint(texel))) , 0, 0, 1);
}
uint4 FUNC_OVERLOAD OCLRASTER_FUNC image_read_uint_nearest(global const uchar* img, const float2 coord) {
 const uint2 img_size = oclr_get_image_size((image_header_ptr)img);
 const float2 fimg_size = convert_float2(img_size) - 1.0f;
 const float2 norm_coord = fmod(coord + fabs(floor(coord)), (float2)(1.0f, 1.0f));
 const uint2 ui_tc = clamp(convert_uint2(norm_coord * fimg_size), (uint2)(0u, 0u), img_size - 1u);
...
uint4 FUNC_OVERLOAD OCLRASTER_FUNC image_read_uint_nearest(global const uchar* img, const uint2 coord) {
 const uint2 img_size = oclr_get_image_size((image_header_ptr)img);
 return image_read_uint_nearest(img, coord.y * img_size.x + coord.x);
}
uint4 FUNC_OVERLOAD OCLRASTER_FUNC image_read_uint_linear(global const uchar* img, const float2 coord) {
 const uint2 img_size = oclr_get_image_size((image_header_ptr)img);
 global const uchar* img_data_ptr = (global const uchar*)((global const uchar*)img + OCLRASTER_IMAGE_HEADER_SIZE);
 const float2 fimg_size = convert_float2(img_size) - 1.0f;
 const float2 norm_coord = fmod(coord + fabs(floor(coord)), (float2)(1.0f, 1.0f));
...
void FUNC_OVERLOAD OCLRASTER_FUNC image_write_uint(global uchar* img, const uint2 coord, const uint4 color) {
 const uint2 img_size = oclr_get_image_size((image_header_ptr)img);
 const uint offset = coord.y * img_size.x + coord.x;
 global uchar* img_data_ptr = (global uchar*)((global uchar*)img + OCLRASTER_IMAGE_HEADER_SIZE);
...
#if defined(OCLRASTER_IMAGE_UCHAR2)
uint4 FUNC_OVERLOAD OCLRASTER_FUNC image_read_uint_nearest(global const uchar2* img, const uint offset) {
 global const uchar2* img_data_ptr = (global const uchar2*)((global const uchar*)img + OCLRASTER_IMAGE_HEADER_SIZE);
 const uchar2 texel = img_data_ptr[offset];
 return (uint4)( ((convert_uint2(texel))) , 0, 1);
}
uint4 FUNC_OVERLOAD OCLRASTER_FUNC image_read_uint_nearest(global const uchar2* img, const float2 coord) {
 const uint2 img_size = oclr_get_image_size((image_header_ptr)img);
 const float2 fimg_size = convert_float2(img_size) - 1.0f;
 const float2 norm_coord = fmod(coord + fabs(floor(coord)), (float2)(1.0f, 1.0f));
 const uint2 ui_tc = clamp(convert_uint2(norm_coord * fimg_size), (uint2)(0u, 0u), img_size - 1u);
...
uint4 FUNC_OVERLOAD OCLRASTER_FUNC image_read_uint_nearest(global const uchar2* img, const uint2 coord) {
 const uint2 img_size = oclr_get_image_size((image_header_ptr)img);
 return image_read_uint_nearest(img, coord.y * img_size.x + coord.x);
}
uint4 FUNC_OVERLOAD OCLRASTER_FUNC image_read_uint_linear(global const uchar2* img, const float2 coord) {
 const uint2 img_size = oclr_get_image_size((image_header_ptr)img);
 global const uchar2* img_data_ptr = (global const uchar2*)((global const uchar*)img + OCLRASTER_IMAGE_HEADER_SIZE);
 const float2 fimg_size = convert_float2(img_size) - 1.0f;
 const float2 norm_coord = fmod(coord + fabs(floor(coord)), (float2)(1.0f, 1.0f));
...
void FUNC_OVERLOAD OCLRASTER_FUNC image_write_uint(global uchar2* img, const uint2 coord, const uint4 color) {
 const uint2 img_size = oclr_get_image_size((image_header_ptr)img);
 const uint offset = coord.y * img_size.x + coord.x;
 global uchar2* img_data_ptr = (global uchar2*)((global uchar*)img + OCLRASTER_IMAGE_HEADER_SIZE);
...
#if defined(OCLRASTER_IMAGE_UCHAR3)
uint4 FUNC_OVERLOAD OCLRASTER_FUNC image_read_uint_nearest(global const uchar3* img, const uint offset) {
 global const uchar3* img_data_ptr = (global const uchar3*)((global const uchar*)img + OCLRASTER_IMAGE_HEADER_SIZE);
 const uchar3 texel = img_data_ptr[offset];
 return (uint4)( ((convert_uint3(texel))) , 1);
}
uint4 FUNC_OVERLOAD OCLRASTER_FUNC image_read_uint_nearest(global const uchar3* img, const float2 coord) {
 const uint2 img_size = oclr_get_image_size((image_header_ptr)img);
 const float2 fimg_size = convert_float2(img_size) - 1.0f;
 const float2 norm_coord = fmod(coord + fabs(floor(coord)), (float2)(1.0f, 1.0f));
 const uint2 ui_tc = clamp(convert_uint2(norm_coord * fimg_size), (uint2)(0u, 0u), img_size - 1u);
...
uint4 FUNC_OVERLOAD OCLRASTER_FUNC image_read_uint_nearest(global const uchar3* img, const uint2 coord) {
 const uint2 img_size = oclr_get_image_size((image_header_ptr)img);
 return image_read_uint_nearest(img, coord.y * img_size.x + coord.x);
}
uint4 FUNC_OVERLOAD OCLRASTER_FUNC image_read_uint_linear(global const uchar3* img, const float2 coord) {
 const uint2 img_size = oclr_get_image_size((image_header_ptr)img);
 global const uchar3* img_data_ptr = (global const uchar3*)((global const uchar*)img + OCLRASTER_IMAGE_HEADER_SIZE);
 const float2 fimg_size = convert_float2(img_size) - 1.0f;
 const float2 norm_coord = fmod(coord + fabs(floor(coord)), (float2)(1.0f, 1.0f));
...
void FUNC_OVERLOAD OCLRASTER_FUNC image_write_uint(global uchar3* img, const uint2 coord, const uint4 color) {
 const uint2 img_size = oclr_get_image_size((image_header_ptr)img);
 const uint offset = coord.y * img_size.x + coord.x;
 global uchar3* img_data_ptr = (global uchar3*)((global uchar*)img + OCLRASTER_IMAGE_HEADER_SIZE);
...
#if defined(OCLRASTER_IMAGE_UCHAR4)
uint4 FUNC_OVERLOAD OCLRASTER_FUNC image_read_uint_nearest(global const uchar4* img, const uint offset) {
 global const uchar4* img_data_ptr = (global const uchar4*)((global const uchar*)img + OCLRASTER_IMAGE_HEADER_SIZE);
 const uchar4 texel = img_data_ptr[offset];
 return (uint4)( ((convert_uint4(texel))) );
}
uint4 FUNC_OVERLOAD OCLRASTER_FUNC image_read_uint_nearest(global const uchar4* img, const float2 coord) {
 const uint2 img_size = oclr_get_image_size((image_header_ptr)img);
 const float2 fimg_size = convert_float2(img_size) - 1.0f;
 const float2 norm_coord = fmod(coord + fabs(floor(coord)), (float2)(1.0f, 1.0f));
 const uint2 ui_tc = clamp(convert_uint2(norm_coord * fimg_size), (uint2)(0u, 0u), img_size - 1u);
...
uint4 FUNC_OVERLOAD OCLRASTER_FUNC image_read_uint_nearest(global const uchar4* img, const uint2 coord) {
 const uint2 img_size = oclr_get_image_size((image_header_ptr)img);
 return image_read_uint_nearest(img, coord.y * img_size.x + coord.x);
}
uint4 FUNC_OVERLOAD OCLRASTER_FUNC image_read_uint_linear(global const uchar4* img, const float2 coord) {
 const uint2 img_size = oclr_get_image_size((image_header_ptr)img);
 global const uchar4* img_data_ptr = (global const uchar4*)((global const uchar*)img + OCLRASTER_IMAGE_HEADER_SIZE);
 const float2 fimg_size = convert_float2(img_size) - 1.0f;
 const float2 norm_coord = fmod(coord + fabs(floor(coord)), (float2)(1.0f, 1.0f));
...
void FUNC_OVERLOAD OCLRASTER_FUNC image_write_uint(global uchar4* img, const uint2 coord, const uint4 color) {
 const uint2 img_size = oclr_get_image_size((image_header_ptr)img);
 const uint offset = coord.y * img_size.x + coord.x;
 global uchar4* img_data_ptr = (global uchar4*)((global uchar*)img + OCLRASTER_IMAGE_HEADER_SIZE);
...
float4 FUNC_OVERLOAD OCLRASTER_FUNC image_read_float_nearest(global const ushort* img, const uint offset) {
...
float4 FUNC_OVERLOAD OCLRASTER_FUNC image_read_float_nearest(global const ushort* img, const float2 coord) {
 const uint2 img_size = oclr_get_image_size((image_header_ptr)img);
 const float2 fimg_size = convert_float2(img_size) - 1.0f;
 const float2 norm_coord = fmod(coord + fabs(floor(coord)), (float2)(1.0f, 1.0f));
 const uint2 ui_tc = clamp(convert_uint2(norm_coord * fimg_size), (uint2)(0u, 0u), img_size - 1u);
...
float4 FUNC_OVERLOAD OCLRASTER_FUNC image_read_float_nearest(global const ushort* img, const uint2 coord) {
 const uint2 img_size = oclr_get_image_size((image_header_ptr)img);
 return image_read_float_nearest(img, coord.y * img_size.x + coord.x);
}
float4 FUNC_OVERLOAD OCLRASTER_FUNC image_read_float_linear(global const ushort* img, const float2 coord) {
 const uint2 img_size = oclr_get_image_size((image_header_ptr)img);
 global const ushort* img_data_ptr = (global const ushort*)((global const uchar*)img + OCLRASTER_IMAGE_HEADER_SIZE);
 const float2 fimg_size = convert_float2(img_size) - 1.0f;
 const float2 norm_coord = fmod(coord + fabs(floor(coord)), (float2)(1.0f, 1.0f));
...
void FUNC_OVERLOAD OCLRASTER_FUNC image_write(global ushort* img, const uint2 coord, const float4 color) {
 const uint2 img_size = oclr_get_image_size((image_header_ptr)img);
 const uint offset = coord.y * img_size.x + coord.x;
...
float4 FUNC_OVERLOAD OCLRASTER_FUNC image_read_float_nearest(global const ushort2* img, const uint offset) {
...
float4 FUNC_OVERLOAD OCLRASTER_FUNC image_read_float_nearest(global const ushort2* img, const float2 coord) {
 const uint2 img_size = oclr_get_image_size((image_header_ptr)img);
 const float2 fimg_size = convert_float2(img_size) - 1.0f;
 const float2 norm_coord = fmod(coord + fabs(floor(coord)), (float2)(1.0f, 1.0f));
 const uint2 ui_tc = clamp(convert_uint2(norm_coord * fimg_size), (uint2)(0u, 0u), img_size - 1u);
...
float4 FUNC_OVERLOAD OCLRASTER_FUNC image_read_float_nearest(global const ushort2* img, const uint2 coord) {
 const uint2 img_size = oclr_get_image_size((image_header_ptr)img);
 return image_read_float_nearest(img, coord.y * img_size.x + coord.x);
}
float4 FUNC_OVERLOAD OCLRASTER_FUNC image_read_float_linear(global const ushort2* img, const float2 coord) {
 const uint2 img_size = oclr_get_image_size((image_header_ptr)img);
 global const ushort2* img_data_ptr = (global const ushort2*)((global const uchar*)img + OCLRASTER_IMAGE_HEADER_SIZE);
 const float2 fimg_size = convert_float2(img_size) - 1.0f;
 const float2 norm_coord = fmod(coord + fabs(floor(coord)), (float2)(1.0f, 1.0f));
...
void FUNC_OVERLOAD OCLRASTER_FUNC image_write(global ushort2* img, const uint2 coord, const float4 color) {
 const uint2 img_size = oclr_get_image_size((image_header_ptr)img);
 const uint offset = coord.y * img_size.x + coord.x;
...
float4 FUNC_OVERLOAD OCLRASTER_FUNC image_read_float_nearest(global const ushort3* img, const uint offset) {
...
float4 FUNC_OVERLOAD OCLRASTER_FUNC image_read_float_nearest(global const ushort3* img, const float2 coord) {
 const uint2 img_size = oclr_get_image_size((image_header_ptr)img);
 const float2 fimg_size = convert_float2(img_size) - 1.0f;
 const float2 norm_coord = fmod(coord + fabs(floor(coord)), (float2)(1.0f, 1.0f));
 const uint2 ui_tc = clamp(convert_uint2(norm_coord * fimg_size), (uint2)(0u, 0u), img_size - 1u);
...
float4 FUNC_OVERLOAD OCLRASTER_FUNC image_read_float_nearest(global const ushort3* img, const uint2 coord) {
 const uint2 img_size = oclr_get_image_size((image_header_ptr)img);
 return image_read_float_nearest(img, coord.y * img_size.x + coord.x);
}
float4 FUNC_OVERLOAD OCLRASTER_FUNC image_read_float_linear(global const ushort3* img, const float2 coord) {
 const uint2 img_size = oclr_get_image_size((image_header_ptr)img);
 global const ushort3* img_data_ptr = (global const ushort3*)((global const uchar*)img + OCLRASTER_IMAGE_HEADER_SIZE);
 const float2 fimg_size = convert_float2(img_size) - 1.0f;
 const float2 norm_coord = fmod(coord + fabs(floor(coord)), (float2)(1.0f, 1.0f));
...
void FUNC_OVERLOAD OCLRASTER_FUNC image_write(global ushort3* img, const uint2 coord, const float4 color) {
 const uint2 img_size = oclr_get_image_size((image_header_ptr)img);
 const uint offset = coord.y * img_size.x + coord.x;
...
float4 FUNC_OVERLOAD OCLRASTER_FUNC image_read_float_nearest(global const ushort4* img, const uint offset) {
...
float4 FUNC_OVERLOAD OCLRASTER_FUNC image_read_float_nearest(global const ushort4* img, const float2 coord) {
 const uint2 img_size = oclr_get_image_size((image_header_ptr)img);
 const float2 fimg_size = convert_float2(img_size) - 1.0f;
 const float2 norm_coord = fmod(coord + fabs(floor(coord)), (float2)(1.0f, 1.0f));
 const uint2 ui_tc = clamp(convert_uint2(norm_coord * fimg_size), (uint2)(0u, 0u), img_size - 1u);
...
float4 FUNC_OVERLOAD OCLRASTER_FUNC image_read_float_nearest(global const ushort4* img, const uint2 coord) {
 const uint2 img_size = oclr_get_image_size((image_header_ptr)img);
 return image_read_float_nearest(img, coord.y * img_size.x + coord.x);
}
float4 FUNC_OVERLOAD OCLRASTER_FUNC image_read_float_linear(global const ushort4* img, const float2 coord) {
 const uint2 img_size = oclr_get_image_size((image_header_ptr)img);
 global const ushort4* img_data_ptr = (global const ushort4*)((global const uchar*)img + OCLRASTER_IMAGE_HEADER_SIZE);
 const float2 fimg_size = convert_float2(img_size) - 1.0f;
 const float2 norm_coord = fmod(coord + fabs(floor(coord)), (float2)(1.0f, 1.0f));
...
void FUNC_OVERLOAD OCLRASTER_FUNC image_write(global ushort4* img, const uint2 coord, const float4 color) {
 const uint2 img_size = oclr_get_image_size((image_header_ptr)img);
 const uint offset = coord.y * img_size.x + coord.x;
...
#if defined(OCLRASTER_IMAGE_USHORT)
uint4 FUNC_OVERLOAD OCLRASTER_FUNC image_read_uint_nearest(global const ushort* img, const uint offset) {
 global const ushort* img_data_ptr = (global const ushort*)((global const uchar*)img + OCLRASTER_IMAGE_HEADER_SIZE);
 const ushort texel = img_data_ptr[offset];
 return (uint4)( ((convert_uint(texel))) , 0, 0, 1);
}
uint4 FUNC_OVERLOAD OCLRASTER_FUNC image_read_uint_nearest(global const ushort* img, const float2 coord) {
 const uint2 img_size = oclr_get_image_size((image_header_ptr)img);
 const float2 fimg_size = convert_float2(img_size) - 1.0f;
 const float2 norm_coord = fmod(coord + fabs(floor(coord)), (float2)(1.0f, 1.0f));
 const uint2 ui_tc = clamp(convert_uint2(norm_coord * fimg_size), (uint2)(0u, 0u), img_size - 1u);
...
uint4 FUNC_OVERLOAD OCLRASTER_FUNC image_read_uint_nearest(global const ushort* img, const uint2 coord) {
 const uint2 img_size = oclr_get_image_size((image_header_ptr)img);
 return image_read_uint_nearest(img, coord.y * img_size.x + coord.x);
}
uint4 FUNC_OVERLOAD OCLRASTER_FUNC image_read_uint_linear(global const ushort* img, const float2 coord) {
 const uint2 img_size = oclr_get_image_size((image_header_ptr)img);
 global const ushort* img_data_ptr = (global const ushort*)((global const uchar*)img + OCLRASTER_IMAGE_HEADER_SIZE);
 const float2 fimg_size = convert_float2(img_size) - 1.0f;
 const float2 norm_coord = fmod(coord + fabs(floor(coord)), (float2)(1.0f, 1.0f));
...
void FUNC_OVERLOAD OCLRASTER_FUNC image_write_uint(global ushort* img, const uint2 coord, const uint4 color) {
 const uint2 img_size = oclr_get_image_size((image_header_ptr)img);
 const uint offset = coord.y * img_size.x + coord.x;
 global ushort* img_data_ptr = (global ushort*)((global uchar*)img + OCLRASTER_IMAGE_HEADER_SIZE);
...
#if defined(OCLRASTER_IMAGE_USHORT2)
uint4 FUNC_OVERLOAD OCLRASTER_FUNC image_read_uint_nearest(global const ushort2* img, const uint offset) {
 global const ushort2* img_data_ptr = (global const ushort2*)((global const uchar*)img + OCLRASTER_IMAGE_HEADER_SIZE);
 const ushort2 texel = img_data_ptr[offset];
 return (uint4)( ((convert_uint2(texel))) , 0, 1);
}
uint4 FUNC_OVERLOAD OCLRASTER_FUNC image_read_uint_nearest(global const ushort2* img, const float2 coord) {
 const uint2 img_size = oclr_get_image_size((image_header_ptr)img);
 const float2 fimg_size = convert_float2(img_size) - 1.0f;
 const float2 norm_coord = fmod(coord + fabs(floor(coord)), (float2)(1.0f, 1.0f));
 const uint2 ui_tc = clamp(convert_uint2(norm_coord * fimg_size), (uint2)(0u, 0u), img_size - 1u);
...
uint4 FUNC_OVERLOAD OCLRASTER_FUNC image_read_uint_nearest(global const ushort2* img, const uint2 coord) {
 const uint2 img_size = oclr_get_image_size((image_header_ptr)img);
 return image_read_uint_nearest(img, coord.y * img_size.x + coord.x);
}
uint4 FUNC_OVERLOAD OCLRASTER_FUNC image_read_uint_linear(global const ushort2* img, const float2 coord) {
 const uint2 img_size = oclr_get_image_size((image_header_ptr)img);
 global const ushort2* img_data_ptr = (global const ushort2*)((global const uchar*)img + OCLRASTER_IMAGE_HEADER_SIZE);
 const float2 fimg_size = convert_float2(img_size) - 1.0f;
 const float2 norm_coord = fmod(coord + fabs(floor(coord)), (float2)(1.0f, 1.0f));
...
void FUNC_OVERLOAD OCLRASTER_FUNC image_write_uint(global ushort2* img, const uint2 coord, const uint4 color) {
 const uint2 img_size = oclr_get_image_size((image_header_ptr)img);
 const uint offset = coord.y * img_size.x + coord.x;
 global ushort2* img_data_ptr = (global ushort2*)((global uchar*)img + OCLRASTER_IMAGE_HEADER_SIZE);
...
#if defined(OCLRASTER_IMAGE_USHORT3)
uint4 FUNC_OVERLOAD OCLRASTER_FUNC image_read_uint_nearest(global const ushort3* img, const uint offset) {
 global const ushort3* img_data_ptr = (global const ushort3*)((global const uchar*)img + OCLRASTER_IMAGE_HEADER_SIZE);
 const ushort3 texel = img_data_ptr[offset];
 return (uint4)( ((convert_uint3(texel))) , 1);
}
uint4 FUNC_OVERLOAD OCLRASTER_FUNC image_read_uint_nearest(global const ushort3* img, const float2 coord) {
 const uint2 img_size = oclr_get_image_size((image_header_ptr)img);
 const float2 fimg_size = convert_float2(img_size) - 1.0f;
 const float2 norm_coord = fmod(coord + fabs(floor(coord)), (float2)(1.0f, 1.0f));
 const uint2 ui_tc = clamp(convert_uint2(norm_coord * fimg_size), (uint2)(0u, 0u), img_size - 1u);
...
uint4 FUNC_OVERLOAD OCLRASTER_FUNC image_read_uint_nearest(global const ushort3* img, const uint2 coord) {
 const uint2 img_size = oclr_get_image_size((image_header_ptr)img);
 return image_read_uint_nearest(img, coord.y * img_size.x + coord.x);
}
uint4 FUNC_OVERLOAD OCLRASTER_FUNC image_read_uint_linear(global const ushort3* img, const float2 coord) {
 const uint2 img_size = oclr_get_image_size((image_header_ptr)img);
 global const ushort3* img_data_ptr = (global const ushort3*)((global const uchar*)img + OCLRASTER_IMAGE_HEADER_SIZE);
 const float2 fimg_size = convert_float2(img_size) - 1.0f;
 const float2 norm_coord = fmod(coord + fabs(floor(coord)), (float2)(1.0f, 1.0f));
...
void FUNC_OVERLOAD OCLRASTER_FUNC image_write_uint(global ushort3* img, const uint2 coord, const uint4 color) {
 const uint2 img_size = oclr_get_image_size((image_header_ptr)img);
 const uint offset = coord.y * img_size.x + coord.x;
 global ushort3* img_data_ptr = (global ushort3*)((global uchar*)img + OCLRASTER_IMAGE_HEADER_SIZE);
...
#if defined(OCLRASTER_IMAGE_USHORT4)
uint4 FUNC_OVERLOAD OCLRASTER_FUNC image_read_uint_nearest(global const ushort4* img, const uint offset) {
 global const ushort4* img_data_ptr = (global const ushort4*)((global const uchar*)img + OCLRASTER_IMAGE_HEADER_SIZE);
 const ushort4 texel = img_data_ptr[offset];
 return (uint4)( ((convert_uint4(texel))) );
}
uint4 FUNC_OVERLOAD OCLRASTER_FUNC image_read_uint_nearest(global const ushort4* img, const float2 coord) {
 const uint2 img_size = oclr_get_image_size((image_header_ptr)img);
 const float2 fimg_size = convert_float2(img_size) - 1.0f;
 const float2 norm_coord = fmod(coord + fabs(floor(coord)), (float2)(1.0f, 1.0f));
 const uint2 ui_tc = clamp(convert_uint2(norm_coord * fimg_size), (uint2)(0u, 0u), img_size - 1u);
...
uint4 FUNC_OVERLOAD OCLRASTER_FUNC image_read_uint_nearest(global const ushort4* img, const uint2 coord) {
 const uint2 img_size = oclr_get_image_size((image_header_ptr)img);
 return image_read_uint_nearest(img, coord.y * img_size.x + coord.x);
}
uint4 FUNC_OVERLOAD OCLRASTER_FUNC image_read_uint_linear(global const ushort4* img, const float2 coord) {
 const uint2 img_size = oclr_get_image_size((image_header_ptr)img);
 global const ushort4* img_data_ptr = (global const ushort4*)((global const uchar*)img + OCLRASTER_IMAGE_HEADER_SIZE);
 const float2 fimg_size = convert_float2(img_size) - 1.0f;
 const float2 norm_coord = fmod(coord + fabs(floor(coord)), (float2)(1.0f, 1.0f));
...
void FUNC_OVERLOAD OCLRASTER_FUNC image_write_uint(global ushort4* img, const uint2 coord, const uint4 color) {
 const uint2 img_size = oclr_get_image_size((image_header_ptr)img);
 const uint offset = coord.y * img_size.x + coord.x;
 global ushort4* img_data_ptr = (global ushort4*)((global uchar*)img + OCLRASTER_IMAGE_HEADER_SIZE);
...
uint4 FUNC_OVERLOAD OCLRASTER_FUNC image_read_uint_nearest(global const uint* img, const uint offset) {
...
uint4 FUNC_OVERLOAD OCLRASTER_FUNC image_read_uint_nearest(global const uint* img, const float2 coord) {
 const uint2 img_size = oclr_get_image_size((image_header_ptr)img);
 const float2 fimg_size = convert_float2(img_size) - 1.0f;
 const float2 norm_coord = fmod(coord + fabs(floor(coord)), (float2)(1.0f, 1.0f));
 const uint2 ui_tc = clamp(convert_uint2(norm_coord * fimg_size), (uint2)(0u, 0u), img_size - 1u);
...
uint4 FUNC_OVERLOAD OCLRASTER_FUNC image_read_uint_nearest(global const uint* img, const uint2 coord) {
 const uint2 img_size = oclr_get_image_size((image_header_ptr)img);
 return image_read_uint_nearest(img, coord.y * img_size.x + coord.x);
}
uint4 FUNC_OVERLOAD OCLRASTER_FUNC image_read_uint_linear(global const uint* img, const float2 coord) {
 const uint2 img_size = oclr_get_image_size((image_header_ptr)img);
 global const uint* img_data_ptr = (global const uint*)((global const uchar*)img + OCLRASTER_IMAGE_HEADER_SIZE);
 const float2 fimg_size = convert_float2(img_size) - 1.0f;
 const float2 norm_coord = fmod(coord + fabs(floor(coord)), (float2)(1.0f, 1.0f));
...
void FUNC_OVERLOAD OCLRASTER_FUNC image_write_uint(global uint* img, const uint2 coord, const uint4 color) {
 const uint2 img_size = oclr_get_image_size((image_header_ptr)img);
 const uint offset = coord.y * img_size.x + coord.x;
...
uint4 FUNC_OVERLOAD OCLRASTER_FUNC image_read_uint_nearest(global const uint2* img, const uint offset) {
...
uint4 FUNC_OVERLOAD OCLRASTER_FUNC image_read_uint_nearest(global const uint2* img, const float2 coord) {
 const uint2 img_size = oclr_get_image_size((image_header_ptr)img);
 const float2 fimg_size = convert_float2(img_size) - 1.0f;
 const float2 norm_coord = fmod(coord + fabs(floor(coord)), (float2)(1.0f, 1.0f));
 const uint2 ui_tc = clamp(convert_uint2(norm_coord * fimg_size), (uint2)(0u, 0u), img_size - 1u);
...
uint4 FUNC_OVERLOAD OCLRASTER_FUNC image_read_uint_nearest(global const uint2* img, const uint2 coord) {
 const uint2 img_size = oclr_get_image_size((image_header_ptr)img);
 return image_read_uint_nearest(img, coord.y * img_size.x + coord.x);
}
uint4 FUNC_OVERLOAD OCLRASTER_FUNC image_read_uint_linear(global const uint2* img, const float2 coord) {
 const uint2 img_size = oclr_get_image_size((image_header_ptr)img);
 global const uint2* img_data_ptr = (global const uint2*)((global const uchar*)img + OCLRASTER_IMAGE_HEADER_SIZE);
 const float2 fimg_size = convert_float2(img_size) - 1.0f;
 const float2 norm_coord = fmod(coord + fabs(floor(coord)), (float2)(1.0f, 1.0f));
...
void FUNC_OVERLOAD OCLRASTER_FUNC image_write_uint(global uint2* img, const uint2 coord, const uint4 color) {
 const uint2 img_size = oclr_get_image_size((image_header_ptr)img);
 const uint offset = coord.y * img_size.x + coord.x;
...
uint4 FUNC_OVERLOAD OCLRASTER_FUNC image_read_uint_nearest(global const uint3* img, const uint offset) {
...
uint4 FUNC_OVERLOAD OCLRASTER_FUNC image_read_uint_nearest(global const uint3* img, const float2 coord) {
 const uint2 img_size = oclr_get_image_size((image_header_ptr)img);
 const float2 fimg_size = convert_float2(img_size) - 1.0f;
 const float2 norm_coord = fmod(coord + fabs(floor(coord)), (float2)(1.0f, 1.0f));
 const uint2 ui_tc = clamp(convert_uint2(norm_coord * fimg_size), (uint2)(0u, 0u), img_size - 1u);
...
uint4 FUNC_OVERLOAD OCLRASTER_FUNC image_read_uint_nearest(global const uint3* img, const uint2 coord) {
 const uint2 img_size = oclr_get_image_size((image_header_ptr)img);
 return image_read_uint_nearest(img, coord.y * img_size.x + coord.x);
}
uint4 FUNC_OVERLOAD OCLRASTER_FUNC image_read_uint_linear(global const uint3* img, const float2 coord) {
 const uint2 img_size = oclr_get_image_size((image_header_ptr)img);
 global const uint3* img_data_ptr = (global const uint3*)((global const uchar*)img + OCLRASTER_IMAGE_HEADER_SIZE);
 const float2 fimg_size = convert_float2(img_size) - 1.0f;
 const float2 norm_coord = fmod(coord + fabs(floor(coord)), (float2)(1.0f, 1.0f));
...
void FUNC_OVERLOAD OCLRASTER_FUNC image_write_uint(global uint3* img, const uint2 coord, const uint4 color) {
 const uint2 img_size = oclr_get_image_size((image_header_ptr)img);
 const uint offset = coord.y * img_size.x + coord.x;
...
uint4 FUNC_OVERLOAD OCLRASTER_FUNC image_read_uint_nearest(global const uint4* img, const uint offset) {
...
uint4 FUNC_OVERLOAD OCLRASTER_FUNC image_read_uint_nearest(global const uint4* img, const float2 coord) {
 const uint2 img_size = oclr_get_image_size((image_header_ptr)img);
 const float2 fimg_size = convert_float2(img_size) - 1.0f;
 const float2 norm_coord = fmod(coord + fabs(floor(coord)), (float2)(1.0f, 1.0f));
 const uint2 ui_tc = clamp(convert_uint2(norm_coord * fimg_size), (uint2)(0u, 0u), img_size - 1u);
...
uint4 FUNC_OVERLOAD OCLRASTER_FUNC image_read_uint_nearest(global const uint4* img, const uint2 coord) {
 const uint2 img_size = oclr_get_image_size((image_header_ptr)img);
 return image_read_uint_nearest(img, coord.y * img_size.x + coord.x);
}
uint4 FUNC_OVERLOAD OCLRASTER_FUNC image_read_uint_linear(global const uint4* img, const float2 coord) {
 const uint2 img_size = oclr_get_image_size((image_header_ptr)img);
 global const uint4* img_data_ptr = (global const uint4*)((global const uchar*)img + OCLRASTER_IMAGE_HEADER_SIZE);
 const float2 fimg_size = convert_float2(img_size) - 1.0f;
 const float2 norm_coord = fmod(coord + fabs(floor(coord)), (float2)(1.0f, 1.0f));
...
void FUNC_OVERLOAD OCLRASTER_FUNC image_write_uint(global uint4* img, const uint2 coord, const uint4 color) {
 const uint2 img_size = oclr_get_image_size((image_header_ptr)img);
 const uint offset = coord.y * img_size.x + coord.x;
...
ulong4 FUNC_OVERLOAD OCLRASTER_FUNC image_read_ulong_nearest(global const ulong* img, const uint offset) {
...
ulong4 FUNC_OVERLOAD OCLRASTER_FUNC image_read_ulong_nearest(global const ulong* img, const float2 coord) {
 const uint2 img_size = oclr_get_image_size((image_header_ptr)img);
 const float2 fimg_size = convert_float2(img_size) - 1.0f;
 const float2 norm_coord = fmod(coord + fabs(floor(coord)), (float2)(1.0f, 1.0f));
 const uint2 ui_tc = clamp(convert_uint2(norm_coord * fimg_size), (uint2)(0u, 0u), img_size - 1u);
...
ulong4 FUNC_OVERLOAD OCLRASTER_FUNC image_read_ulong_nearest(global const ulong* img, const uint2 coord) {
 const uint2 img_size = oclr_get_image_size((image_header_ptr)img);
 return image_read_ulong_nearest(img, coord.y * img_size.x + coord.x);
}
ulong4 FUNC_OVERLOAD OCLRASTER_FUNC image_read_ulong_linear(global const ulong* img, const float2 coord) {
 const uint2 img_size = oclr_get_image_size((image_header_ptr)img);
 global const ulong* img_data_ptr = (global const ulong*)((global const uchar*)img + OCLRASTER_IMAGE_HEADER_SIZE);
 const float2 fimg_size = convert_float2(img_size) - 1.0f;
 const float2 norm_coord = fmod(coord + fabs(floor(coord)), (float2)(1.0f, 1.0f));
...
void FUNC_OVERLOAD OCLRASTER_FUNC image_write_ulong(global ulong* img, const uint2 coord, const ulong4 color) {
 const uint2 img_size = oclr_get_image_size((image_header_ptr)img);
 const uint offset = coord.y * img_size.x + coord.x;
...
ulong4 FUNC_OVERLOAD OCLRASTER_FUNC image_read_ulong_nearest(global const ulong2* img, const uint offset) {
...
ulong4 FUNC_OVERLOAD OCLRASTER_FUNC image_read_ulong_nearest(global const ulong2* img, const float2 coord) {
 const uint2 img_size = oclr_get_image_size((image_header_ptr)img);
 const float2 fimg_size = convert_float2(img_size) - 1.0f;
 const float2 norm_coord = fmod(coord + fabs(floor(coord)), (float2)(1.0f, 1.0f));
 const uint2 ui_tc = clamp(convert_uint2(norm_coord * fimg_size), (uint2)(0u, 0u), img_size - 1u);
...
ulong4 FUNC_OVERLOAD OCLRASTER_FUNC image_read_ulong_nearest(global const ulong2* img, const uint2 coord) {
 const uint2 img_size = oclr_get_image_size((image_header_ptr)img);
 return image_read_ulong_nearest(img, coord.y * img_size.x + coord.x);
}
ulong4 FUNC_OVERLOAD OCLRASTER_FUNC image_read_ulong_linear(global const ulong2* img, const float2 coord) {
 const uint2 img_size = oclr_get_image_size((image_header_ptr)img);
 global const ulong2* img_data_ptr = (global const ulong2*)((global const uchar*)img + OCLRASTER_IMAGE_HEADER_SIZE);
 const float2 fimg_size = convert_float2(img_size) - 1.0f;
 const float2 norm_coord = fmod(coord + fabs(floor(coord)), (float2)(1.0f, 1.0f));
...
void FUNC_OVERLOAD OCLRASTER_FUNC image_write_ulong(global ulong2* img, const uint2 coord, const ulong4 color) {
 const uint2 img_size = oclr_get_image_size((image_header_ptr)img);
 const uint offset = coord.y * img_size.x + coord.x;
...
ulong4 FUNC_OVERLOAD OCLRASTER_FUNC image_read_ulong_nearest(global const ulong3* img, const uint offset) {
...
ulong4 FUNC_OVERLOAD OCLRASTER_FUNC image_read_ulong_nearest(global const ulong3* img, const float2 coord) {
 const uint2 img_size = oclr_get_image_size((image_header_ptr)img);
 const float2 fimg_size = convert_float2(img_size) - 1.0f;
 const float2 norm_coord = fmod(coord + fabs(floor(coord)), (float2)(1.0f, 1.0f));
 const uint2 ui_tc = clamp(convert_uint2(norm_coord * fimg_size), (uint2)(0u, 0u), img_size - 1u);
...
ulong4 FUNC_OVERLOAD OCLRASTER_FUNC image_read_ulong_nearest(global const ulong3* img, const uint2 coord) {
 const uint2 img_size = oclr_get_image_size((image_header_ptr)img);
 return image_read_ulong_nearest(img, coord.y * img_size.x + coord.x);
}
ulong4 FUNC_OVERLOAD OCLRASTER_FUNC image_read_ulong_linear(global const ulong3* img, const float2 coord) {
 const uint2 img_size = oclr_get_image_size((image_header_ptr)img);
 global const ulong3* img_data_ptr = (global const ulong3*)((global const uchar*)img + OCLRASTER_IMAGE_HEADER_SIZE);
 const float2 fimg_size = convert_float2(img_size) - 1.0f;
 const float2 norm_coord = fmod(coord + fabs(floor(coord)), (float2)(1.0f, 1.0f));
...
void FUNC_OVERLOAD OCLRASTER_FUNC image_write_ulong(global ulong3* img, const uint2 coord, const ulong4 color) {
 const uint2 img_size = oclr_get_image_size((image_header_ptr)img);
 const uint offset = coord.y * img_size.x + coord.x;
...
ulong4 FUNC_OVERLOAD OCLRASTER_FUNC image_read_ulong_nearest(global const ulong4* img, const uint offset) {
...
ulong4 FUNC_OVERLOAD OCLRASTER_FUNC image_read_ulong_nearest(global const ulong4* img, const float2 coord) {
 const uint2 img_size = oclr_get_image_size((image_header_ptr)img);
 const float2 fimg_size = convert_float2(img_size) - 1.0f;
 const float2 norm_coord = fmod(coord + fabs(floor(coord)), (float2)(1.0f, 1.0f));
 const uint2 ui_tc = clamp(convert_uint2(norm_coord * fimg_size), (uint2)(0u, 0u), img_size - 1u);
...
ulong4 FUNC_OVERLOAD OCLRASTER_FUNC image_read_ulong_nearest(global const ulong4* img, const uint2 coord) {
 const uint2 img_size = oclr_get_image_size((image_header_ptr)img);
 return image_read_ulong_nearest(img, coord.y * img_size.x + coord.x);
}
ulong4 FUNC_OVERLOAD OCLRASTER_FUNC image_read_ulong_linear(global const ulong4* img, const float2 coord) {
 const uint2 img_size = oclr_get_image_size((image_header_ptr)img);
 global const ulong4* img_data_ptr = (global const ulong4*)((global const uchar*)img + OCLRASTER_IMAGE_HEADER_SIZE);
 const float2 fimg_size = convert_float2(img_size) - 1.0f;
 const float2 norm_coord = fmod(coord + fabs(floor(coord)), (float2)(1.0f, 1.0f));
...
void FUNC_OVERLOAD OCLRASTER_FUNC image_write_ulong(global ulong4* img, const uint2 coord, const ulong4 color) {
 const uint2 img_size = oclr_get_image_size((image_header_ptr)img);
 const uint offset = coord.y * img_size.x + coord.x;
...
float4 FUNC_OVERLOAD OCLRASTER_FUNC image_read_float_nearest(global const char* img, const uint offset) {
...
float4 FUNC_OVERLOAD OCLRASTER_FUNC image_read_float_nearest(global const char* img, const float2 coord) {
 const uint2 img_size = oclr_get_image_size((image_header_ptr)img);
 const float2 fimg_size = convert_float2(img_size) - 1.0f;
 const float2 norm_coord = fmod(coord + fabs(floor(coord)), (float2)(1.0f, 1.0f));
 const uint2 ui_tc = clamp(convert_uint2(norm_coord * fimg_size), (uint2)(0u, 0u), img_size - 1u);
...
float4 FUNC_OVERLOAD OCLRASTER_FUNC image_read_float_nearest(global const char* img, const uint2 coord) {
 const uint2 img_size = oclr_get_image_size((image_header_ptr)img);
 return image_read_float_nearest(img, coord.y * img_size.x + coord.x);
}
float4 FUNC_OVERLOAD OCLRASTER_FUNC image_read_float_linear(global const char* img, const float2 coord) {
 const uint2 img_size = oclr_get_image_size((image_header_ptr)img);
 global const char* img_data_ptr = (global const char*)((global const uchar*)img + OCLRASTER_IMAGE_HEADER_SIZE);
 const float2 fimg_size = convert_float2(img_size) - 1.0f;
 const float2 norm_coord = fmod(coord + fabs(floor(coord)), (float2)(1.0f, 1.0f));
...
void FUNC_OVERLOAD OCLRASTER_FUNC image_write(global char* img, const uint2 coord, const float4 color) {
 const uint2 img_size = oclr_get_image_size((image_header_ptr)img);
 const uint offset = coord.y * img_size.x + coord.x;
...
float4 FUNC_OVERLOAD OCLRASTER_FUNC image_read_float_nearest(global const char2* img, const uint offset) {
...
float4 FUNC_OVERLOAD OCLRASTER_FUNC image_read_float_nearest(global const char2* img, const float2 coord) {
 const uint2 img_size = oclr_get_image_size((image_header_ptr)img);
 const float2 fimg_size = convert_float2(img_size) - 1.0f;
 const float2 norm_coord = fmod(coord + fabs(floor(coord)), (float2)(1.0f, 1.0f));
 const uint2 ui_tc = clamp(convert_uint2(norm_coord * fimg_size), (uint2)(0u, 0u), img_size - 1u);
...
float4 FUNC_OVERLOAD OCLRASTER_FUNC image_read_float_nearest(global const char2* img, const uint2 coord) {
 const uint2 img_size = oclr_get_image_size((image_header_ptr)img);
 return image_read_float_nearest(img, coord.y * img_size.x + coord.x);
}
float4 FUNC_OVERLOAD OCLRASTER_FUNC image_read_float_linear(global const char2* img, const float2 coord) {
 const uint2 img_size = oclr_get_image_size((image_header_ptr)img);
 global const char2* img_data_ptr = (global const char2*)((global const uchar*)img + OCLRASTER_IMAGE_HEADER_SIZE);
 const float2 fimg_size = convert_float2(img_size) - 1.0f;
 const float2 norm_coord = fmod(coord + fabs(floor(coord)), (float2)(1.0f, 1.0f));
...
void FUNC_OVERLOAD OCLRASTER_FUNC image_write(global char2* img, const uint2 coord, const float4 color) {
 const uint2 img_size = oclr_get_image_size((image_header_ptr)img);
 const uint offset = coord.y * img_size.x + coord.x;
...
float4 FUNC_OVERLOAD OCLRASTER_FUNC image_read_float_nearest(global const char3* img, const uint offset) {
...
float4 FUNC_OVERLOAD OCLRASTER_FUNC image_read_float_nearest(global const char3* img, const float2 coord) {
 const uint2 img_size = oclr_get_image_size((image_header_ptr)img);
 const float2 fimg_size = convert_float2(img_size) - 1.0f;
 const float2 norm_coord = fmod(coord + fabs(floor(coord)), (float2)(1.0f, 1.0f));
 const uint2 ui_tc = clamp(convert_uint2(norm_coord * fimg_size), (uint2)(0u, 0u), img_size - 1u);
...
float4 FUNC_OVERLOAD OCLRASTER_FUNC image_read_float_nearest(global const char3* img, const uint2 coord) {
 const uint2 img_size = oclr_get_image_size((image_header_ptr)img);
 return image_read_float_nearest(img, coord.y * img_size.x + coord.x);
}
float4 FUNC_OVERLOAD OCLRASTER_FUNC image_read_float_linear(global const char3* img, const float2 coord) {
 const uint2 img_size = oclr_get_image_size((image_header_ptr)img);
 global const char3* img_data_ptr = (global const char3*)((global const uchar*)img + OCLRASTER_IMAGE_HEADER_SIZE);
 const float2 fimg_size = convert_float2(img_size) - 1.0f;
 const float2 norm_coord = fmod(coord + fabs(floor(coord)), (float2)(1.0f, 1.0f));
...
void FUNC_OVERLOAD OCLRASTER_FUNC image_write(global char3* img, const uint2 coord, const float4 color) {
 const uint2 img_size = oclr_get_image_size((image_header_ptr)img);
 const uint offset = coord.y * img_size.x + coord.x;
...
float4 FUNC_OVERLOAD OCLRASTER_FUNC image_read_float_nearest(global const char4* img, const uint offset) {
...
float4 FUNC_OVERLOAD OCLRASTER_FUNC image_read_float_nearest(global const char4* img, const float2 coord) {
 const uint2 img_size = oclr_get_image_size((image_header_ptr)img);
 const float2 fimg_size = convert_float2(img_size) - 1.0f;
 const float2 norm_coord = fmod(coord + fabs(floor(coord)), (float2)(1.0f, 1.0f));
 const uint2 ui_tc = clamp(convert_uint2(norm_coord * fimg_size), (uint2)(0u, 0u), img_size - 1u);
...
float4 FUNC_OVERLOAD OCLRASTER_FUNC image_read_float_nearest(global const char4* img, const uint2 coord) {
 const uint2 img_size = oclr_get_image_size((image_header_ptr)img);
 return image_read_float_nearest(img, coord.y * img_size.x + coord.x);
}
float4 FUNC_OVERLOAD OCLRASTER_FUNC image_read_float_linear(global const char4* img, const float2 coord) {
 const uint2 img_size = oclr_get_image_size((image_header_ptr)img);
 global const char4* img_data_ptr = (global const char4*)((global const uchar*)img + OCLRASTER_IMAGE_HEADER_SIZE);
 const float2 fimg_size = convert_float2(img_size) - 1.0f;
 const float2 norm_coord = fmod(coord + fabs(floor(coord)), (float2)(1.0f, 1.0f));
...
void FUNC_OVERLOAD OCLRASTER_FUNC image_write(global char4* img, const uint2 coord, const float4 color) {
 const uint2 img_size = oclr_get_image_size((image_header_ptr)img);
 const uint offset = coord.y * img_size.x + coord.x;
...
#if defined(OCLRASTER_IMAGE_CHAR)
int4 FUNC_OVERLOAD OCLRASTER_FUNC image_read_int_nearest(global const char* img, const uint offset) {
 global const char* img_data_ptr = (global const char*)((global const uchar*)img + OCLRASTER_IMAGE_HEADER_SIZE);
 const char texel = img_data_ptr[offset];
 return (int4)( ((convert_int(texel))) , 0, 0, 1);
}
int4 FUNC_OVERLOAD OCLRASTER_FUNC image_read_int_nearest(global const char* img, const float2 coord) {
 const uint2 img_size = oclr_get_image_size((image_header_ptr)img);
 const float2 fimg_size = convert_float2(img_size) - 1.0f;
 const float2 norm_coord = fmod(coord + fabs(floor(coord)), (float2)(1.0f, 1.0f));
 const uint2 ui_tc = clamp(convert_uint2(norm_coord * fimg_size), (uint2)(0u, 0u), img_size - 1u);
...
int4 FUNC_OVERLOAD OCLRASTER_FUNC image_read_int_nearest(global const char* img, const uint2 coord) {
 const uint2 img_size = oclr_get_image_size((image_header_ptr)img);
 return image_read_int_nearest(img, coord.y * img_size.x + coord.x);
}
int4 FUNC_OVERLOAD OCLRASTER_FUNC image_read_int_linear(global const char* img, const float2 coord) {
 const uint2 img_size = oclr_get_image_size((image_header_ptr)img);
 global const char* img_data_ptr = (global const char*)((global const uchar*)img + OCLRASTER_IMAGE_HEADER_SIZE);
 const float2 fimg_size = convert_float2(img_size) - 1.0f;
 const float2 norm_coord = fmod(coord + fabs(floor(coord)), (float2)(1.0f, 1.0f));
...
void FUNC_OVERLOAD OCLRASTER_FUNC image_write_int(global char* img, const uint2 coord, const int4 color) {
 const uint2 img_size = oclr_get_image_size((image_header_ptr)img);
 const uint offset = coord.y * img_size.x + coord.x;
 global char* img_data_ptr = (global char*)((global uchar*)img + OCLRASTER_IMAGE_HEADER_SIZE);
...
#if defined(OCLRASTER_IMAGE_CHAR2)
int4 FUNC_OVERLOAD OCLRASTER_FUNC image_read_int_nearest(global const char2* img, const uint offset) {
 global const char2* img_data_ptr = (global const char2*)((global const uchar*)img + OCLRASTER_IMAGE_HEADER_SIZE);
 const char2 texel = img_data_ptr[offset];
 return (int4)( ((convert_int2(texel))) , 0, 1);
}
int4 FUNC_OVERLOAD OCLRASTER_FUNC image_read_int_nearest(global const char2* img, const float2 coord) {
 const uint2 img_size = oclr_get_image_size((image_header_ptr)img);
 const float2 fimg_size = convert_float2(img_size) - 1.0f;
 const float2 norm_coord = fmod(coord + fabs(floor(coord)), (float2)(1.0f, 1.0f));
 const uint2 ui_tc = clamp(convert_uint2(norm_coord * fimg_size), (uint2)(0u, 0u), img_size - 1u);
...
int4 FUNC_OVERLOAD OCLRASTER_FUNC image_read_int_nearest(global const char2* img, const uint2 coord) {
 const uint2 img_size = oclr_get_image_size((image_header_ptr)img);
 return image_read_int_nearest(img, coord.y * img_size.x + coord.x);
}
int4 FUNC_OVERLOAD OCLRASTER_FUNC image_read_int_linear(global const char2* img, const float2 coord) {
 const uint2 img_size = oclr_get_image_size((image_header_ptr)img);
 global const char2* img_data_ptr = (global const char2*)((global const uchar*)img + OCLRASTER_IMAGE_HEADER_SIZE);
 const float2 fimg_size = convert_float2(img_size) - 1.0f;
 const float2 norm_coord = fmod(coord + fabs(floor(coord)), (float2)(1.0f, 1.0f));
...
void FUNC_OVERLOAD OCLRASTER_FUNC image_write_int(global char2* img, const uint2 coord, const int4 color) {
 const uint2 img_size = oclr_get_image_size((image_header_ptr)img);
 const uint offset = coord.y * img_size.x + coord.x;
 global char2* img_data_ptr = (global char2*)((global uchar*)img + OCLRASTER_IMAGE_HEADER_SIZE);
...
#if defined(OCLRASTER_IMAGE_CHAR3)
int4 FUNC_OVERLOAD OCLRASTER_FUNC image_read_int_nearest(global const char3* img, const uint offset) {
 global const char3* img_data_ptr = (global const char3*)((global const uchar*)img + OCLRASTER_IMAGE_HEADER_SIZE);
 const char3 texel = img_data_ptr[offset];
 return (int4)( ((convert_int3(texel))) , 1);
}
int4 FUNC_OVERLOAD OCLRASTER_FUNC image_read_int_nearest(global const char3* img, const float2 coord) {
 const uint2 img_size = oclr_get_image_size((image_header_ptr)img);
 const float2 fimg_size = convert_float2(img_size) - 1.0f;
 const float2 norm_coord = fmod(coord + fabs(floor(coord)), (float2)(1.0f, 1.0f));
 const uint2 ui_tc = clamp(convert_uint2(norm_coord * fimg_size), (uint2)(0u, 0u), img_size - 1u);
...
int4 FUNC_OVERLOAD OCLRASTER_FUNC image_read_int_nearest(global const char3* img, const uint2 coord) {
 const uint2 img_size = oclr_get_image_size((image_header_ptr)img);
 return image_read_int_nearest(img, coord.y * img_size.x + coord.x);
}
int4 FUNC_OVERLOAD OCLRASTER_FUNC image_read_int_linear(global const char3* img, const float2 coord) {
 const uint2 img_size = oclr_get_image_size((image_header_ptr)img);
 global const char3* img_data_ptr = (global const char3*)((global const uchar*)img + OCLRASTER_IMAGE_HEADER_SIZE);
 const float2 fimg_size = convert_float2(img_size) - 1.0f;
 const float2 norm_coord = fmod(coord + fabs(floor(coord)), (float2)(1.0f, 1.0f));
...
void FUNC_OVERLOAD OCLRASTER_FUNC image_write_int(global char3* img, const uint2 coord, const int4 color) {
 const uint2 img_size = oclr_get_image_size((image_header_ptr)img);
 const uint offset = coord.y * img_size.x + coord.x;
 global char3* img_data_ptr = (global char3*)((global uchar*)img + OCLRASTER_IMAGE_HEADER_SIZE);
...
#if defined(OCLRASTER_IMAGE_CHAR4)
int4 FUNC_OVERLOAD OCLRASTER_FUNC image_read_int_nearest(global const char4* img, const uint offset) {
 global const char4* img_data_ptr = (global const char4*)((global const uchar*)img + OCLRASTER_IMAGE_HEADER_SIZE);
 const char4 texel = img_data_ptr[offset];
 return (int4)( ((convert_int4(texel))) );
}
int4 FUNC_OVERLOAD OCLRASTER_FUNC image_read_int_nearest(global const char4* img, const float2 coord) {
 const uint2 img_size = oclr_get_image_size((image_header_ptr)img);
 const float2 fimg_size = convert_float2(img_size) - 1.0f;
 const float2 norm_coord = fmod(coord + fabs(floor(coord)), (float2)(1.0f, 1.0f));
 const uint2 ui_tc = clamp(convert_uint2(norm_coord * fimg_size), (uint2)(0u, 0u), img_size - 1u);
...
int4 FUNC_OVERLOAD OCLRASTER_FUNC image_read_int_nearest(global const char4* img, const uint2 coord) {
 const uint2 img_size = oclr_get_image_size((image_header_ptr)img);
 return image_read_int_nearest(img, coord.y * img_size.x + coord.x);
}
int4 FUNC_OVERLOAD OCLRASTER_FUNC image_read_int_linear(global const char4* img, const float2 coord) {
 const uint2 img_size = oclr_get_image_size((image_header_ptr)img);
 global const char4* img_data_ptr = (global const char4*)((global const uchar*)img + OCLRASTER_IMAGE_HEADER_SIZE);
 const float2 fimg_size = convert_float2(img_size) - 1.0f;
 const float2 norm_coord = fmod(coord + fabs(floor(coord)), (float2)(1.0f, 1.0f));
...
void FUNC_OVERLOAD OCLRASTER_FUNC image_write_int(global char4* img, const uint2 coord, const int4 color) {
 const uint2 img_size = oclr_get_image_size((image_header_ptr)img);
 const uint offset = coord.y * img_size.x + coord.x;
 global char4* img_data_ptr = (global char4*)((global uchar*)img + OCLRASTER_IMAGE_HEADER_SIZE);
...
float4 FUNC_OVERLOAD OCLRASTER_FUNC image_read_float_nearest(global const short* img, const uint offset) {
...
float4 FUNC_OVERLOAD OCLRASTER_FUNC image_read_float_nearest(global const short* img, const float2 coord) {
 const uint2 img_size = oclr_get_image_size((image_header_ptr)img);
 const float2 fimg_size = convert_float2(img_size) - 1.0f;
 const float2 norm_coord = fmod(coord + fabs(floor(coord)), (float2)(1.0f, 1.0f));
 const uint2 ui_tc = clamp(convert_uint2(norm_coord * fimg_size), (uint2)(0u, 0u), img_size - 1u);
...
float4 FUNC_OVERLOAD OCLRASTER_FUNC image_read_float_nearest(global const short* img, const uint2 coord) {
 const uint2 img_size = oclr_get_image_size((image_header_ptr)img);
 return image_read_float_nearest(img, coord.y * img_size.x + coord.x);
}
float4 FUNC_OVERLOAD OCLRASTER_FUNC image_read_float_linear(global const short* img, const float2 coord) {
 const uint2 img_size = oclr_get_image_size((image_header_ptr)img);
 global const short* img_data_ptr = (global const short*)((global const uchar*)img + OCLRASTER_IMAGE_HEADER_SIZE);
 const float2 fimg_size = convert_float2(img_size) - 1.0f;
 const float2 norm_coord = fmod(coord + fabs(floor(coord)), (float2)(1.0f, 1.0f));
...
void FUNC_OVERLOAD OCLRASTER_FUNC image_write(global short* img, const uint2 coord, const float4 color) {
 const uint2 img_size = oclr_get_image_size((image_header_ptr)img);
 const uint offset = coord.y * img_size.x + coord.x;
...
float4 FUNC_OVERLOAD OCLRASTER_FUNC image_read_float_nearest(global const short2* img, const uint offset) {
...
float4 FUNC_OVERLOAD OCLRASTER_FUNC image_read_float_nearest(global const short2* img, const float2 coord) {
 const uint2 img_size = oclr_get_image_size((image_header_ptr)img);
 const float2 fimg_size = convert_float2(img_size) - 1.0f;
 const float2 norm_coord = fmod(coord + fabs(floor(coord)), (float2)(1.0f, 1.0f));
 const uint2 ui_tc = clamp(convert_uint2(norm_coord * fimg_size), (uint2)(0u, 0u), img_size - 1u);
...
float4 FUNC_OVERLOAD OCLRASTER_FUNC image_read_float_nearest(global const short2* img, const uint2 coord) {
 const uint2 img_size = oclr_get_image_size((image_header_ptr)img);
 return image_read_float_nearest(img, coord.y * img_size.x + coord.x);
}
float4 FUNC_OVERLOAD OCLRASTER_FUNC image_read_float_linear(global const short2* img, const float2 coord) {
 const uint2 img_size = oclr_get_image_size((image_header_ptr)img);
 global const short2* img_data_ptr = (global const short2*)((global const uchar*)img + OCLRASTER_IMAGE_HEADER_SIZE);
 const float2 fimg_size = convert_float2(img_size) - 1.0f;
 const float2 norm_coord = fmod(coord + fabs(floor(coord)), (float2)(1.0f, 1.0f));
...
void FUNC_OVERLOAD OCLRASTER_FUNC image_write(global short2* img, const uint2 coord, const float4 color) {
 const uint2 img_size = oclr_get_image_size((image_header_ptr)img);
 const uint offset = coord.y * img_size.x + coord.x;
...
float4 FUNC_OVERLOAD OCLRASTER_FUNC image_read_float_nearest(global const short3* img, const uint offset) {
...
float4 FUNC_OVERLOAD OCLRASTER_FUNC image_read_float_nearest(global const short3* img, const float2 coord) {
 const uint2 img_size = oclr_get_image_size((image_header_ptr)img);
 const float2 fimg_size = convert_float2(img_size) - 1.0f;
 const float2 norm_coord = fmod(coord + fabs(floor(coord)), (float2)(1.0f, 1.0f));
 const uint2 ui_tc = clamp(convert_uint2(norm_coord * fimg_size), (uint2)(0u, 0u), img_size - 1u);
...
float4 FUNC_OVERLOAD OCLRASTER_FUNC image_read_float_nearest(global const short3* img, const uint2 coord) {
 const uint2 img_size = oclr_get_image_size((image_header_ptr)img);
 return image_read_float_nearest(img, coord.y * img_size.x + coord.x);
}
float4 FUNC_OVERLOAD OCLRASTER_FUNC image_read_float_linear(global const short3* img, const float2 coord) {
 const uint2 img_size = oclr_get_image_size((image_header_ptr)img);
 global const short3* img_data_ptr = (global const short3*)((global const uchar*)img + OCLRASTER_IMAGE_HEADER_SIZE);
 const float2 fimg_size = convert_float2(img_size) - 1.0f;
 const float2 norm_coord = fmod(coord + fabs(floor(coord)), (float2)(1.0f, 1.0f));
...
void FUNC_OVERLOAD OCLRASTER_FUNC image_write(global short3* img, const uint2 coord, const float4 color) {
 const uint2 img_size = oclr_get_image_size((image_header_ptr)img);
 const uint offset = coord.y * img_size.x + coord.x;
...
float4 FUNC_OVERLOAD OCLRASTER_FUNC image_read_float_nearest(global const short4* img, const uint offset) {
...
float4 FUNC_OVERLOAD OCLRASTER_FUNC image_read_float_nearest(global const short4* img, const float2 coord) {
 const uint2 img_size = oclr_get_image_size((image_header_ptr)img);
 const float2 fimg_size = convert_float2(img_size) - 1.0f;
 const float2 norm_coord = fmod(coord + fabs(floor(coord)), (float2)(1.0f, 1.0f));
 const uint2 ui_tc = clamp(convert_uint2(norm_coord * fimg_size), (uint2)(0u, 0u), img_size - 1u);
...
float4 FUNC_OVERLOAD OCLRASTER_FUNC image_read_float_nearest(global const short4* img, const uint2 coord) {
 const uint2 img_size = oclr_get_image_size((image_header_ptr)img);
 return image_read_float_nearest(img, coord.y * img_size.x + coord.x);
}
float4 FUNC_OVERLOAD OCLRASTER_FUNC image_read_float_linear(global const short4* img, const float2 coord) {
 const uint2 img_size = oclr_get_image_size((image_header_ptr)img);
 global const short4* img_data_ptr = (global const short4*)((global const uchar*)img + OCLRASTER_IMAGE_HEADER_SIZE);
 const float2 fimg_size = convert_float2(img_size) - 1.0f;
 const float2 norm_coord = fmod(coord + fabs(floor(coord)), (float2)(1.0f, 1.0f));
...
void FUNC_OVERLOAD OCLRASTER_FUNC image_write(global short4* img, const uint2 coord, const float4 color) {
 const uint2 img_size = oclr_get_image_size((image_header_ptr)img);
 const uint offset = coord.y * img_size.x + coord.x;
...
#if defined(OCLRASTER_IMAGE_SHORT)
int4 FUNC_OVERLOAD OCLRASTER_FUNC image_read_int_nearest(global const short* img, const uint offset) {
 global const short* img_data_ptr = (global const short*)((global const uchar*)img + OCLRASTER_IMAGE_HEADER_SIZE);
 const short texel = img_data_ptr[offset];
 return (int4)( ((convert_int(texel))) , 0, 0, 1);
}
int4 FUNC_OVERLOAD OCLRASTER_FUNC image_read_int_nearest(global const short* img, const float2 coord) {
 const uint2 img_size = oclr_get_image_size((image_header_ptr)img);
 const float2 fimg_size = convert_float2(img_size) - 1.0f;
 const float2 norm_coord = fmod(coord + fabs(floor(coord)), (float2)(1.0f, 1.0f));
 const uint2 ui_tc = clamp(convert_uint2(norm_coord * fimg_size), (uint2)(0u, 0u), img_size - 1u);
...
int4 FUNC_OVERLOAD OCLRASTER_FUNC image_read_int_nearest(global const short* img, const uint2 coord) {
 const uint2 img_size = oclr_get_image_size((image_header_ptr)img);
 return image_read_int_nearest(img, coord.y * img_size.x + coord.x);
}
int4 FUNC_OVERLOAD OCLRASTER_FUNC image_read_int_linear(global const short* img, const float2 coord) {
 const uint2 img_size = oclr_get_image_size((image_header_ptr)img);
 global const short* img_data_ptr = (global const short*)((global const uchar*)img + OCLRASTER_IMAGE_HEADER_SIZE);
 const float2 fimg_size = convert_float2(img_size) - 1.0f;
 const float2 norm_coord = fmod(coord + fabs(floor(coord)), (float2)(1.0f, 1.0f));
...
void FUNC_OVERLOAD OCLRASTER_FUNC image_write_int(global short* img, const uint2 coord, const int4 color) {
 const uint2 img_size = oclr_get_image_size((image_header_ptr)img);
 const uint offset = coord.y * img_size.x + coord.x;
 global short* img_data_ptr = (global short*)((global uchar*)img + OCLRASTER_IMAGE_HEADER_SIZE);
...
#if defined(OCLRASTER_IMAGE_SHORT2)
int4 FUNC_OVERLOAD OCLRASTER_FUNC image_read_int_nearest(global const short2* img, const uint offset) {
 global const short2* img_data_ptr = (global const short2*)((global const uchar*)img + OCLRASTER_IMAGE_HEADER_SIZE);
 const short2 texel = img_data_ptr[offset];
 return (int4)( ((convert_int2(texel))) , 0, 1);
}
int4 FUNC_OVERLOAD OCLRASTER_FUNC image_read_int_nearest(global const short2* img, const float2 coord) {
 const uint2 img_size = oclr_get_image_size((image_header_ptr)img);
 const float2 fimg_size = convert_float2(img_size) - 1.0f;
 const float2 norm_coord = fmod(coord + fabs(floor(coord)), (float2)(1.0f, 1.0f));
 const uint2 ui_tc = clamp(convert_uint2(norm_coord * fimg_size), (uint2)(0u, 0u), img_size - 1u);
...
int4 FUNC_OVERLOAD OCLRASTER_FUNC image_read_int_nearest(global const short2* img, const uint2 coord) {
 const uint2 img_size = oclr_get_image_size((image_header_ptr)img);
 return image_read_int_nearest(img, coord.y * img_size.x + coord.x);
}
int4 FUNC_OVERLOAD OCLRASTER_FUNC image_read_int_linear(global const short2* img, const float2 coord) {
 const uint2 img_size = oclr_get_image_size((image_header_ptr)img);
 global const short2* img_data_ptr = (global const short2*)((global const uchar*)img + OCLRASTER_IMAGE_HEADER_SIZE);
 const float2 fimg_size = convert_float2(img_size) - 1.0f;
 const float2 norm_coord = fmod(coord + fabs(floor(coord)), (float2)(1.0f, 1.0f));
...
void FUNC_OVERLOAD OCLRASTER_FUNC image_write_int(global short2* img, const uint2 coord, const int4 color) {
 const uint2 img_size = oclr_get_image_size((image_header_ptr)img);
 const uint offset = coord.y * img_size.x + coord.x;
 global short2* img_data_ptr = (global short2*)((global uchar*)img + OCLRASTER_IMAGE_HEADER_SIZE);
...
#if defined(OCLRASTER_IMAGE_SHORT3)
int4 FUNC_OVERLOAD OCLRASTER_FUNC image_read_int_nearest(global const short3* img, const uint offset) {
 global const short3* img_data_ptr = (global const short3*)((global const uchar*)img + OCLRASTER_IMAGE_HEADER_SIZE);
 const short3 texel = img_data_ptr[offset];
 return (int4)( ((convert_int3(texel))) , 1);
}
int4 FUNC_OVERLOAD OCLRASTER_FUNC image_read_int_nearest(global const short3* img, const float2 coord) {
 const uint2 img_size = oclr_get_image_size((image_header_ptr)img);
 const float2 fimg_size = convert_float2(img_size) - 1.0f;
 const float2 norm_coord = fmod(coord + fabs(floor(coord)), (float2)(1.0f, 1.0f));
 const uint2 ui_tc = clamp(convert_uint2(norm_coord * fimg_size), (uint2)(0u, 0u), img_size - 1u);
...
int4 FUNC_OVERLOAD OCLRASTER_FUNC image_read_int_nearest(global const short3* img, const uint2 coord) {
 const uint2 img_size = oclr_get_image_size((image_header_ptr)img);
 return image_read_int_nearest(img, coord.y * img_size.x + coord.x);
}
int4 FUNC_OVERLOAD OCLRASTER_FUNC image_read_int_linear(global const short3* img, const float2 coord) {
 const uint2 img_size = oclr_get_image_size((image_header_ptr)img);
 global const short3* img_data_ptr = (global const short3*)((global const uchar*)img + OCLRASTER_IMAGE_HEADER_SIZE);
 const float2 fimg_size = convert_float2(img_size) - 1.0f;
 const float2 norm_coord = fmod(coord + fabs(floor(coord)), (float2)(1.0f, 1.0f));
...
void FUNC_OVERLOAD OCLRASTER_FUNC image_write_int(global short3* img, const uint2 coord, const int4 color) {
 const uint2 img_size = oclr_get_image_size((image_header_ptr)img);
 const uint offset = coord.y * img_size.x + coord.x;
 global short3* img_data_ptr = (global short3*)((global uchar*)img + OCLRASTER_IMAGE_HEADER_SIZE);
...
#if defined(OCLRASTER_IMAGE_SHORT4)
int4 FUNC_OVERLOAD OCLRASTER_FUNC image_read_int_nearest(global const short4* img, const uint offset) {
 global const short4* img_data_ptr = (global const short4*)((global const uchar*)img + OCLRASTER_IMAGE_HEADER_SIZE);
 const short4 texel = img_data_ptr[offset];
 return (int4)( ((convert_int4(texel))) );
}
int4 FUNC_OVERLOAD OCLRASTER_FUNC image_read_int_nearest(global const short4* img, const float2 coord) {
 const uint2 img_size = oclr_get_image_size((image_header_ptr)img);
 const float2 fimg_size = convert_float2(img_size) - 1.0f;
 const float2 norm_coord = fmod(coord + fabs(floor(coord)), (float2)(1.0f, 1.0f));
 const uint2 ui_tc = clamp(convert_uint2(norm_coord * fimg_size), (uint2)(0u, 0u), img_size - 1u);
...
int4 FUNC_OVERLOAD OCLRASTER_FUNC image_read_int_nearest(global const short4* img, const uint2 coord) {
 const uint2 img_size = oclr_get_image_size((image_header_ptr)img);
 return image_read_int_nearest(img, coord.y * img_size.x + coord.x);
}
int4 FUNC_OVERLOAD OCLRASTER_FUNC image_read_int_linear(global const short4* img, const float2 coord) {
 const uint2 img_size = oclr_get_image_size((image_header_ptr)img);
 global const short4* img_data_ptr = (global const short4*)((global const uchar*)img + OCLRASTER_IMAGE_HEADER_SIZE);
 const float2 fimg_size = convert_float2(img_size) - 1.0f;
 const float2 norm_coord = fmod(coord + fabs(floor(coord)), (float2)(1.0f, 1.0f));
...
void FUNC_OVERLOAD OCLRASTER_FUNC image_write_int(global short4* img, const uint2 coord, const int4 color) {
 const uint2 img_size = oclr_get_image_size((image_header_ptr)img);
 const uint offset = coord.y * img_size.x + coord.x;
 global short4* img_data_ptr = (global short4*)((global uchar*)img + OCLRASTER_IMAGE_HEADER_SIZE);
...
int4 FUNC_OVERLOAD OCLRASTER_FUNC image_read_int_nearest(global const int* img, const uint offset) {
...
int4 FUNC_OVERLOAD OCLRASTER_FUNC image_read_int_nearest(global const int* img, const float2 coord) {
 const uint2 img_size = oclr_get_image_size((image_header_ptr)img);
 const float2 fimg_size = convert_float2(img_size) - 1.0f;
 const float2 norm_coord = fmod(coord + fabs(floor(coord)), (float2)(1.0f, 1.0f));
 const uint2 ui_tc = clamp(convert_uint2(norm_coord * fimg_size), (uint2)(0u, 0u), img_size - 1u);
...
int4 FUNC_OVERLOAD OCLRASTER_FUNC image_read_int_nearest(global const int* img, const uint2 coord) {
 const uint2 img_size = oclr_get_image_size((image_header_ptr)img);
 return image_read_int_nearest(img, coord.y * img_size.x + coord.x);
}
int4 FUNC_OVERLOAD OCLRASTER_FUNC image_read_int_linear(global const int* img, const float2 coord) {
 const uint2 img_size = oclr_get_image_size((image_header_ptr)img);
 global const int* img_data_ptr = (global const int*)((global const uchar*)img + OCLRASTER_IMAGE_HEADER_SIZE);
 const float2 fimg_size = convert_float2(img_size) - 1.0f;
 const float2 norm_coord = fmod(coord + fabs(floor(coord)), (float2)(1.0f, 1.0f));
...
void FUNC_OVERLOAD OCLRASTER_FUNC image_write_int(global int* img, const uint2 coord, const int4 color) {
 const uint2 img_size = oclr_get_image_size((image_header_ptr)img);
 const uint offset = coord.y * img_size.x + coord.x;
...
int4 FUNC_OVERLOAD OCLRASTER_FUNC image_read_int_nearest(global const int2* img, const uint offset) {
...
int4 FUNC_OVERLOAD OCLRASTER_FUNC image_read_int_nearest(global const int2* img, const float2 coord) {
 const uint2 img_size = oclr_get_image_size((image_header_ptr)img);
 const float2 fimg_size = convert_float2(img_size) - 1.0f;
 const float2 norm_coord = fmod(coord + fabs(floor(coord)), (float2)(1.0f, 1.0f));
 const uint2 ui_tc = clamp(convert_uint2(norm_coord * fimg_size), (uint2)(0u, 0u), img_size - 1u);
...
int4 FUNC_OVERLOAD OCLRASTER_FUNC image_read_int_nearest(global const int2* img, const uint2 coord) {
 const uint2 img_size = oclr_get_image_size((image_header_ptr)img);
 return image_read_int_nearest(img, coord.y * img_size.x + coord.x);
}
int4 FUNC_OVERLOAD OCLRASTER_FUNC image_read_int_linear(global const int2* img, const float2 coord) {
 const uint2 img_size = oclr_get_image_size((image_header_ptr)img);
 global const int2* img_data_ptr = (global const int2*)((global const uchar*)img + OCLRASTER_IMAGE_HEADER_SIZE);
 const float2 fimg_size = convert_float2(img_size) - 1.0f;
 const float2 norm_coord = fmod(coord + fabs(floor(coord)), (float2)(1.0f, 1.0f));
...
void FUNC_OVERLOAD OCLRASTER_FUNC image_write_int(global int2* img, const uint2 coord, const int4 color) {
 const uint2 img_size = oclr_get_image_size((image_header_ptr)img);
 const uint offset = coord.y * img_size.x + coord.x;
...
int4 FUNC_OVERLOAD OCLRASTER_FUNC image_read_int_nearest(global const int3* img, const uint offset) {
...
int4 FUNC_OVERLOAD OCLRASTER_FUNC image_read_int_nearest(global const int3* img, const float2 coord) {
 const uint2 img_size = oclr_get_image_size((image_header_ptr)img);
 const float2 fimg_size = convert_float2(img_size) - 1.0f;
 const float2 norm_coord = fmod(coord + fabs(floor(coord)), (float2)(1.0f, 1.0f));
 const uint2 ui_tc = clamp(convert_uint2(norm_coord * fimg_size), (uint2)(0u, 0u), img_size - 1u);
...
int4 FUNC_OVERLOAD OCLRASTER_FUNC image_read_int_nearest(global const int3* img, const uint2 coord) {
 const uint2 img_size = oclr_get_image_size((image_header_ptr)img);
 return image_read_int_nearest(img, coord.y * img_size.x + coord.x);
}
int4 FUNC_OVERLOAD OCLRASTER_FUNC image_read_int_linear(global const int3* img, const float2 coord) {
 const uint2 img_size = oclr_get_image_size((image_header_ptr)img);
 global const int3* img_data_ptr = (global const int3*)((global const uchar*)img + OCLRASTER_IMAGE_HEADER_SIZE);
 const float2 fimg_size = convert_float2(img_size) - 1.0f;
 const float2 norm_coord = fmod(coord + fabs(floor(coord)), (float2)(1.0f, 1.0f));
...
void FUNC_OVERLOAD OCLRASTER_FUNC image_write_int(global int3* img, const uint2 coord, const int4 color) {
 const uint2 img_size = oclr_get_image_size((image_header_ptr)img);
 const uint offset = coord.y * img_size.x + coord.x;
...
int4 FUNC_OVERLOAD OCLRASTER_FUNC image_read_int_nearest(global const int4* img, const uint offset) {
...
int4 FUNC_OVERLOAD OCLRASTER_FUNC image_read_int_nearest(global const int4* img, const float2 coord) {
 const uint2 img_size = oclr_get_image_size((image_header_ptr)img);
 const float2 fimg_size = convert_float2(img_size) - 1.0f;
 const float2 norm_coord = fmod(coord + fabs(floor(coord)), (float2)(1.0f, 1.0f));
 const uint2 ui_tc = clamp(convert_uint2(norm_coord * fimg_size), (uint2)(0u, 0u), img_size - 1u);
...
int4 FUNC_OVERLOAD OCLRASTER_FUNC image_read_int_nearest(global const int4* img, const uint2 coord) {
 const uint2 img_size = oclr_get_image_size((image_header_ptr)img);
 return image_read_int_nearest(img, coord.y * img_size.x + coord.x);
}
int4 FUNC_OVERLOAD OCLRASTER_FUNC image_read_int_linear(global const int4* img, const float2 coord) {
 const uint2 img_size = oclr_get_image_size((image_header_ptr)img);
 global const int4* img_data_ptr = (global const int4*)((global const uchar*)img + OCLRASTER_IMAGE_HEADER_SIZE);
 const float2 fimg_size = convert_float2(img_size) - 1.0f;
 const float2 norm_coord = fmod(coord + fabs(floor(coord)), (float2)(1.0f, 1.0f));
...
void FUNC_OVERLOAD OCLRASTER_FUNC image_write_int(global int4* img, const uint2 coord, const int4 color) {
 const uint2 img_size = oclr_get_image_size((image_header_ptr)img);
 const uint offset = coord.y * img_size.x + coord.x;
...
long4 FUNC_OVERLOAD OCLRASTER_FUNC image_read_long_nearest(global const long* img, const uint offset) {
...
long4 FUNC_OVERLOAD OCLRASTER_FUNC image_read_long_nearest(global const long* img, const float2 coord) {
 const uint2 img_size = oclr_get_image_size((image_header_ptr)img);
 const float2 fimg_size = convert_float2(img_size) - 1.0f;
 const float2 norm_coord = fmod(coord + fabs(floor(coord)), (float2)(1.0f, 1.0f));
 const uint2 ui_tc = clamp(convert_uint2(norm_coord * fimg_size), (uint2)(0u, 0u), img_size - 1u);
...
long4 FUNC_OVERLOAD OCLRASTER_FUNC image_read_long_nearest(global const long* img, const uint2 coord) {
 const uint2 img_size = oclr_get_image_size((image_header_ptr)img);
 return image_read_long_nearest(img, coord.y * img_size.x + coord.x);
}
long4 FUNC_OVERLOAD OCLRASTER_FUNC image_read_long_linear(global const long* img, const float2 coord) {
 const uint2 img_size = oclr_get_image_size((image_header_ptr)img);
 global const long* img_data_ptr = (global const long*)((global const uchar*)img + OCLRASTER_IMAGE_HEADER_SIZE);
 const float2 fimg_size = convert_float2(img_size) - 1.0f;
 const float2 norm_coord = fmod(coord + fabs(floor(coord)), (float2)(1.0f, 1.0f));
...
void FUNC_OVERLOAD OCLRASTER_FUNC image_write_long(global long* img, const uint2 coord, const long4 color) {
 const uint2 img_size = oclr_get_image_size((image_header_ptr)img);
 const uint offset = coord.y * img_size.x + coord.x;
...
long4 FUNC_OVERLOAD OCLRASTER_FUNC image_read_long_nearest(global const long2* img, const uint offset) {
...
long4 FUNC_OVERLOAD OCLRASTER_FUNC image_read_long_nearest(global const long2* img, const float2 coord) {
 const uint2 img_size = oclr_get_image_size((image_header_ptr)img);
 const float2 fimg_size = convert_float2(img_size) - 1.0f;
 const float2 norm_coord = fmod(coord + fabs(floor(coord)), (float2)(1.0f, 1.0f));
 const uint2 ui_tc = clamp(convert_uint2(norm_coord * fimg_size), (uint2)(0u, 0u), img_size - 1u);
...
long4 FUNC_OVERLOAD OCLRASTER_FUNC image_read_long_nearest(global const long2* img, const uint2 coord) {
 const uint2 img_size = oclr_get_image_size((image_header_ptr)img);
 return image_read_long_nearest(img, coord.y * img_size.x + coord.x);
}
long4 FUNC_OVERLOAD OCLRASTER_FUNC image_read_long_linear(global const long2* img, const float2 coord) {
 const uint2 img_size = oclr_get_image_size((image_header_ptr)img);
 global const long2* img_data_ptr = (global const long2*)((global const uchar*)img + OCLRASTER_IMAGE_HEADER_SIZE);
 const float2 fimg_size = convert_float2(img_size) - 1.0f;
 const float2 norm_coord = fmod(coord + fabs(floor(coord)), (float2)(1.0f, 1.0f));
...
void FUNC_OVERLOAD OCLRASTER_FUNC image_write_long(global long2* img, const uint2 coord, const long4 color) {
 const uint2 img_size = oclr_get_image_size((image_header_ptr)img);
 const uint offset = coord.y * img_size.x + coord.x;
...
long4 FUNC_OVERLOAD OCLRASTER_FUNC image_read_long_nearest(global const long3* img, const uint offset) {
...
long4 FUNC_OVERLOAD OCLRASTER_FUNC image_read_long_nearest(global const long3* img, const float2 coord) {
 const uint2 img_size = oclr_get_image_size((image_header_ptr)img);
 const float2 fimg_size = convert_float2(img_size) - 1.0f;
 const float2 norm_coord = fmod(coord + fabs(floor(coord)), (float2)(1.0f, 1.0f));
 const uint2 ui_tc = clamp(convert_uint2(norm_coord * fimg_size), (uint2)(0u, 0u), img_size - 1u);
...
long4 FUNC_OVERLOAD OCLRASTER_FUNC image_read_long_nearest(global const long3* img, const uint2 coord) {
 const uint2 img_size = oclr_get_image_size((image_header_ptr)img);
 return image_read_long_nearest(img, coord.y * img_size.x + coord.x);
}
long4 FUNC_OVERLOAD OCLRASTER_FUNC image_read_long_linear(global const long3* img, const float2 coord) {
 const uint2 img_size = oclr_get_image_size((image_header_ptr)img);
 global const long3* img_data_ptr = (global const long3*)((global const uchar*)img + OCLRASTER_IMAGE_HEADER_SIZE);
 const float2 fimg_size = convert_float2(img_size) - 1.0f;
 const float2 norm_coord = fmod(coord + fabs(floor(coord)), (float2)(1.0f, 1.0f));
...
void FUNC_OVERLOAD OCLRASTER_FUNC image_write_long(global long3* img, const uint2 coord, const long4 color) {
 const uint2 img_size = oclr_get_image_size((image_header_ptr)img);
 const uint offset = coord.y * img_size.x + coord.x;
...
long4 FUNC_OVERLOAD OCLRASTER_FUNC image_read_long_nearest(global const long4* img, const uint offset) {
...
long4 FUNC_OVERLOAD OCLRASTER_FUNC image_read_long_nearest(global const long4* img, const float2 coord) {
 const uint2 img_size = oclr_get_image_size((image_header_ptr)img);
 const float2 fimg_size = convert_float2(img_size) - 1.0f;
 const float2 norm_coord = fmod(coord + fabs(floor(coord)), (float2)(1.0f, 1.0f));
 const uint2 ui_tc = clamp(convert_uint2(norm_coord * fimg_size), (uint2)(0u, 0u), img_size - 1u);
...
long4 FUNC_OVERLOAD OCLRASTER_FUNC image_read_long_nearest(global const long4* img, const uint2 coord) {
 const uint2 img_size = oclr_get_image_size((image_header_ptr)img);
 return image_read_long_nearest(img, coord.y * img_size.x + coord.x);
}
long4 FUNC_OVERLOAD OCLRASTER_FUNC image_read_long_linear(global const long4* img, const float2 coord) {
 const uint2 img_size = oclr_get_image_size((image_header_ptr)img);
 global const long4* img_data_ptr = (global const long4*)((global const uchar*)img + OCLRASTER_IMAGE_HEADER_SIZE);
 const float2 fimg_size = convert_float2(img_size) - 1.0f;
 const float2 norm_coord = fmod(coord + fabs(floor(coord)), (float2)(1.0f, 1.0f));
...
void FUNC_OVERLOAD OCLRASTER_FUNC image_write_long(global long4* img, const uint2 coord, const long4 color) {
 const uint2 img_size = oclr_get_image_size((image_header_ptr)img);
 const uint offset = coord.y * img_size.x + coord.x;
...
float4 FUNC_OVERLOAD OCLRASTER_FUNC image_read_float_nearest(global const oclr_half* img, const uint offset) {
...
float4 FUNC_OVERLOAD OCLRASTER_FUNC image_read_float_nearest(global const oclr_half* img, const float2 coord) {
 const uint2 img_size = oclr_get_image_size((image_header_ptr)img);
 const float2 fimg_size = convert_float2(img_size) - 1.0f;
 const float2 norm_coord = fmod(coord + fabs(floor(coord)), (float2)(1.0f, 1.0f));
 const uint2 ui_tc = clamp(convert_uint2(norm_coord * fimg_size), (uint2)(0u, 0u), img_size - 1u);
...
float4 FUNC_OVERLOAD OCLRASTER_FUNC image_read_float_nearest(global const oclr_half* img, const uint2 coord) {
 const uint2 img_size = oclr_get_image_size((image_header_ptr)img);
 return image_read_float_nearest(img, coord.y * img_size.x + coord.x);
}
float4 FUNC_OVERLOAD OCLRASTER_FUNC image_read_float_linear(global const oclr_half* img, const float2 coord) {
 const uint2 img_size = oclr_get_image_size((image_header_ptr)img);
 global const half* img_data_ptr = (global const half*)((global const uchar*)img + OCLRASTER_IMAGE_HEADER_SIZE);
 const float2 fimg_size = convert_float2(img_size) - 1.0f;
 const float2 norm_coord = fmod(coord + fabs(floor(coord)), (float2)(1.0f, 1.0f));
...
void FUNC_OVERLOAD OCLRASTER_FUNC image_write(global oclr_half* img, const uint2 coord, const float4 color) {
 const uint2 img_size = oclr_get_image_size((image_header_ptr)img);
 const uint offset = coord.y * img_size.x + coord.x;
...
float4 FUNC_OVERLOAD OCLRASTER_FUNC image_read_float_nearest(global const oclr_half2* img, const uint offset) {
 global const half* img_data_ptr = (global const half*)((global const uchar*)img + OCLRASTER_IMAGE_HEADER_SIZE);
 const float2 texel = vload_half2(offset, img_data_ptr);
 return (float4)(texel , 0.0f, 1.0f);
}
float4 FUNC_OVERLOAD OCLRASTER_FUNC image_read_float_nearest(global const oclr_half2* img, const float2 coord) {
 const uint2 img_size = oclr_get_image_size((image_header_ptr)img);
 const float2 fimg_size = convert_float2(img_size) - 1.0f;
 const float2 norm_coord = fmod(coord + fabs(floor(coord)), (float2)(1.0f, 1.0f));
 const uint2 ui_tc = clamp(convert_uint2(norm_coord * fimg_size), (uint2)(0u, 0u), img_size - 1u);
...
float4 FUNC_OVERLOAD OCLRASTER_FUNC image_read_float_nearest(global const oclr_half2* img, const uint2 coord) {
 const uint2 img_size = oclr_get_image_size((image_header_ptr)img);
 return image_read_float_nearest(img, coord.y * img_size.x + coord.x);
}
float4 FUNC_OVERLOAD OCLRASTER_FUNC image_read_float_linear(global const oclr_half2* img, const float2 coord) {
 const uint2 img_size = oclr_get_image_size((image_header_ptr)img);
 global const half* img_data_ptr = (global const half*)((global const uchar*)img + OCLRASTER_IMAGE_HEADER_SIZE);
 const float2 fimg_size = convert_float2(img_size) - 1.0f;
 const float2 norm_coord = fmod(coord + fabs(floor(coord)), (float2)(1.0f, 1.0f));
...
void FUNC_OVERLOAD OCLRASTER_FUNC image_write(global oclr_half2* img, const uint2 coord, const float4 color) {
 const uint2 img_size = oclr_get_image_size((image_header_ptr)img);
 const uint offset = coord.y * img_size.x + coord.x;
...
float4 FUNC_OVERLOAD OCLRASTER_FUNC image_read_float_nearest(global const oclr_half3* img, const uint offset) {
 global const half* img_data_ptr = (global const half*)((global const uchar*)img + OCLRASTER_IMAGE_HEADER_SIZE);
 const float3 texel = vload_half3(offset, img_data_ptr);
 return (float4)(texel , 1.0f);
}
float4 FUNC_OVERLOAD OCLRASTER_FUNC image_read_float_nearest(global const oclr_half3* img, const float2 coord) {
 const uint2 img_size = oclr_get_image_size((image_header_ptr)img);
 const float2 fimg_size = convert_float2(img_size) - 1.0f;
 const float2 norm_coord = fmod(coord + fabs(floor(coord)), (float2)(1.0f, 1.0f));
 const uint2 ui_tc = clamp(convert_uint2(norm_coord * fimg_size), (uint2)(0u, 0u), img_size - 1u);
...
float4 FUNC_OVERLOAD OCLRASTER_FUNC image_read_float_nearest(global const oclr_half3* img, const uint2 coord) {
 const uint2 img_size = oclr_get_image_size((image_header_ptr)img);
 return image_read_float_nearest(img, coord.y * img_size.x + coord.x);
}
float4 FUNC_OVERLOAD OCLRASTER_FUNC image_read_float_linear(global const oclr_half3* img, const float2 coord) {
 const uint2 img_size = oclr_get_image_size((image_header_ptr)img);
 global const half* img_data_ptr = (global const half*)((global const uchar*)img + OCLRASTER_IMAGE_HEADER_SIZE);
 const float2 fimg_size = convert_float2(img_size) - 1.0f;
 const float2 norm_coord = fmod(coord + fabs(floor(coord)), (float2)(1.0f, 1.0f));
...
void FUNC_OVERLOAD OCLRASTER_FUNC image_write(global oclr_half3* img, const uint2 coord, const float4 color) {
 const uint2 img_size = oclr_get_image_size((image_header_ptr)img);
 const uint offset = coord.y * img_size.x + coord.x;
...
float4 FUNC_OVERLOAD OCLRASTER_FUNC image_read_float_nearest(global const oclr_half4* img, const uint offset) {
 global const half* img_data_ptr = (global const half*)((global const uchar*)img + OCLRASTER_IMAGE_HEADER_SIZE);
 const float4 texel = vload_half4(offset, img_data_ptr);
 return (float4)(texel );
}
float4 FUNC_OVERLOAD OCLRASTER_FUNC image_read_float_nearest(global const oclr_half4* img, const float2 coord) {
 const uint2 img_size = oclr_get_image_size((image_header_ptr)img);
 const float2 fimg_size = convert_float2(img_size) - 1.0f;
 const float2 norm_coord = fmod(coord + fabs(floor(coord)), (float2)(1.0f, 1.0f));
 const uint2 ui_tc = clamp(convert_uint2(norm_coord * fimg_size), (uint2)(0u, 0u), img_size - 1u);
...
float4 FUNC_OVERLOAD OCLRASTER_FUNC image_read_float_nearest(global const oclr_half4* img, const uint2 coord) {
 const uint2 img_size = oclr_get_image_size((image_header_ptr)img);
 return image_read_float_nearest(img, coord.y * img_size.x + coord.x);
}
float4 FUNC_OVERLOAD OCLRASTER_FUNC image_read_float_linear(global const oclr_half4* img, const float2 coord) {
 const uint2 img_size = oclr_get_image_size((image_header_ptr)img);
 global const half* img_data_ptr = (global const half*)((global const uchar*)img + OCLRASTER_IMAGE_HEADER_SIZE);
 const float2 fimg_size = convert_float2(img_size) - 1.0f;
 const float2 norm_coord = fmod(coord + fabs(floor(coord)), (float2)(1.0f, 1.0f));
...
void FUNC_OVERLOAD OCLRASTER_FUNC image_write(global oclr_half4* img, const uint2 coord, const float4 color) {
 const uint2 img_size = oclr_get_image_size((image_header_ptr)img);
 const uint offset = coord.y * img_size.x + coord.x;
...
float4 FUNC_OVERLOAD OCLRASTER_FUNC image_read_float_nearest(global const float* img, const uint offset) {
...
float4 FUNC_OVERLOAD OCLRASTER_FUNC image_read_float_nearest(global const float* img, const float2 coord) {
 const uint2 img_size = oclr_get_image_size((image_header_ptr)img);
 const float2 fimg_size = convert_float2(img_size) - 1.0f;
 const float2 norm_coord = fmod(coord + fabs(floor(coord)), (float2)(1.0f, 1.0f));
 const uint2 ui_tc = clamp(convert_uint2(norm_coord * fimg_size), (uint2)(0u, 0u), img_size - 1u);
...
float4 FUNC_OVERLOAD OCLRASTER_FUNC image_read_float_nearest(global const float* img, const uint2 coord) {
 const uint2 img_size = oclr_get_image_size((image_header_ptr)img);
 return image_read_float_nearest(img, coord.y * img_size.x + coord.x);
}
float4 FUNC_OVERLOAD OCLRASTER_FUNC image_read_float_linear(global const float* img, const float2 coord) {
 const uint2 img_size = oclr_get_image_size((image_header_ptr)img);
 global const float* img_data_ptr = (global const float*)((global const uchar*)img + OCLRASTER_IMAGE_HEADER_SIZE);
 const float2 fimg_size = convert_float2(img_size) - 1.0f;
 const float2 norm_coord = fmod(coord + fabs(floor(coord)), (float2)(1.0f, 1.0f));
...
void FUNC_OVERLOAD OCLRASTER_FUNC image_write(global float* img, const uint2 coord, const float4 color) {
 const uint2 img_size = oclr_get_image_size((image_header_ptr)img);
 const uint offset = coord.y * img_size.x + coord.x;
...
float4 FUNC_OVERLOAD OCLRASTER_FUNC image_read_float_nearest(global const float2* img, const uint offset) {
...
float4 FUNC_OVERLOAD OCLRASTER_FUNC image_read_float_nearest(global const float2* img, const float2 coord) {
 const uint2 img_size = oclr_get_image_size((image_header_ptr)img);
 const float2 fimg_size = convert_float2(img_size) - 1.0f;
 const float2 norm_coord = fmod(coord + fabs(floor(coord)), (float2)(1.0f, 1.0f));
 const uint2 ui_tc = clamp(convert_uint2(norm_coord * fimg_size), (uint2)(0u, 0u), img_size - 1u);
...
float4 FUNC_OVERLOAD OCLRASTER_FUNC image_read_float_nearest(global const float2* img, const uint2 coord) {
 const uint2 img_size = oclr_get_image_size((image_header_ptr)img);
 return image_read_float_nearest(img, coord.y * img_size.x + coord.x);
}
float4 FUNC_OVERLOAD OCLRASTER_FUNC image_read_float_linear(global const float2* img, const float2 coord) {
 const uint2 img_size = oclr_get_image_size((image_header_ptr)img);
 global const float2* img_data_ptr = (global const float2*)((global const uchar*)img + OCLRASTER_IMAGE_HEADER_SIZE);
 const float2 fimg_size = convert_float2(img_size) - 1.0f;
 const float2 norm_coord = fmod(coord + fabs(floor(coord)), (float2)(1.0f, 1.0f));
...
void FUNC_OVERLOAD OCLRASTER_FUNC image_write(global float2* img, const uint2 coord, const float4 color) {
 const uint2 img_size = oclr_get_image_size((image_header_ptr)img);
 const uint offset = coord.y * img_size.x + coord.x;
...
float4 FUNC_OVERLOAD OCLRASTER_FUNC image_read_float_nearest(global const float3* img, const uint offset) {
...
float4 FUNC_OVERLOAD OCLRASTER_FUNC image_read_float_nearest(global const float3* img, const float2 coord) {
 const uint2 img_size = oclr_get_image_size((image_header_ptr)img);
 const float2 fimg_size = convert_float2(img_size) - 1.0f;
 const float2 norm_coord = fmod(coord + fabs(floor(coord)), (float2)(1.0f, 1.0f));
 const uint2 ui_tc = clamp(convert_uint2(norm_coord * fimg_size), (uint2)(0u, 0u), img_size - 1u);
...
float4 FUNC_OVERLOAD OCLRASTER_FUNC image_read_float_nearest(global const float3* img, const uint2 coord) {
 const uint2 img_size = oclr_get_image_size((image_header_ptr)img);
 return image_read_float_nearest(img, coord.y * img_size.x + coord.x);
}
float4 FUNC_OVERLOAD OCLRASTER_FUNC image_read_float_linear(global const float3* img, const float2 coord) {
 const uint2 img_size = oclr_get_image_size((image_header_ptr)img);
 global const float3* img_data_ptr = (global const float3*)((global const uchar*)img + OCLRASTER_IMAGE_HEADER_SIZE);
 const float2 fimg_size = convert_float2(img_size) - 1.0f;
 const float2 norm_coord = fmod(coord + fabs(floor(coord)), (float2)(1.0f, 1.0f));
...
void FUNC_OVERLOAD OCLRASTER_FUNC image_write(global float3* img, const uint2 coord, const float4 color) {
 const uint2 img_size = oclr_get_image_size((image_header_ptr)img);
 const uint offset = coord.y * img_size.x + coord.x;
...
float4 FUNC_OVERLOAD OCLRASTER_FUNC image_read_float_nearest(global const float4* img, const uint offset) {
...
float4 FUNC_OVERLOAD OCLRASTER_FUNC image_read_float_nearest(global const float4* img, const float2 coord) {
 const uint2 img_size = oclr_get_image_size((image_header_ptr)img);
 const float2 fimg_size = convert_float2(img_size) - 1.0f;
 const float2 norm_coord = fmod(coord + fabs(floor(coord)), (float2)(1.0f, 1.0f));
 const uint2 ui_tc = clamp(convert_uint2(norm_coord * fimg_size), (uint2)(0u, 0u), img_size - 1u);
...
float4 FUNC_OVERLOAD OCLRASTER_FUNC image_read_float_nearest(global const float4* img, const uint2 coord) {
 const uint2 img_size = oclr_get_image_size((image_header_ptr)img);
 return image_read_float_nearest(img, coord.y * img_size.x + coord.x);
}
float4 FUNC_OVERLOAD OCLRASTER_FUNC image_read_float_linear(global const float4* img, const float2 coord) {
 const uint2 img_size = oclr_get_image_size((image_header_ptr)img);
 global const float4* img_data_ptr = (global const float4*)((global const uchar*)img + OCLRASTER_IMAGE_HEADER_SIZE);
 const float2 fimg_size = convert_float2(img_size) - 1.0f;
 const float2 norm_coord = fmod(coord + fabs(floor(coord)), (float2)(1.0f, 1.0f));
...
void FUNC_OVERLOAD OCLRASTER_FUNC image_write(global float4* img, const uint2 coord, const float4 color) {
 const uint2 img_size = oclr_get_image_size((image_header_ptr)img);
 const uint offset = coord.y * img_size.x + coord.x;
...
double4 FUNC_OVERLOAD OCLRASTER_FUNC image_read_double_nearest(global const double* img, const uint offset) {
...
double4 FUNC_OVERLOAD OCLRASTER_FUNC image_read_double_nearest(global const double* img, const float2 coord) {
 const uint2 img_size = oclr_get_image_size((image_header_ptr)img);
 const float2 fimg_size = convert_float2(img_size) - 1.0f;
 const float2 norm_coord = fmod(coord + fabs(floor(coord)), (float2)(1.0f, 1.0f));
 const uint2 ui_tc = clamp(convert_uint2(norm_coord * fimg_size), (uint2)(0u, 0u), img_size - 1u);
...
double4 FUNC_OVERLOAD OCLRASTER_FUNC image_read_double_nearest(global const double* img, const uint2 coord) {
 const uint2 img_size = oclr_get_image_size((image_header_ptr)img);
 return image_read_double_nearest(img, coord.y * img_size.x + coord.x);
}
double4 FUNC_OVERLOAD OCLRASTER_FUNC image_read_double_linear(global const double* img, const float2 coord) {
 const uint2 img_size = oclr_get_image_size((image_header_ptr)img);
 global const double* img_data_ptr = (global const double*)((global const uchar*)img + OCLRASTER_IMAGE_HEADER_SIZE);
 const float2 fimg_size = convert_float2(img_size) - 1.0f;
 const float2 norm_coord = fmod(coord + fabs(floor(coord)), (float2)(1.0f, 1.0f));
...
void FUNC_OVERLOAD OCLRASTER_FUNC image_write_double(global double* img, const uint2 coord, const double4 color) {
 const uint2 img_size = oclr_get_image_size((image_header_ptr)img);
 const uint offset = coord.y * img_size.x + coord.x;
...
double4 FUNC_OVERLOAD OCLRASTER_FUNC image_read_double_nearest(global const double2* img, const uint offset) {
...
double4 FUNC_OVERLOAD OCLRASTER_FUNC image_read_double_nearest(global const double2* img, const float2 coord) {
 const uint2 img_size = oclr_get_image_size((image_header_ptr)img);
 const float2 fimg_size = convert_float2(img_size) - 1.0f;
 const float2 norm_coord = fmod(coord + fabs(floor(coord)), (float2)(1.0f, 1.0f));
 const uint2 ui_tc = clamp(convert_uint2(norm_coord * fimg_size), (uint2)(0u, 0u), img_size - 1u);
...
double4 FUNC_OVERLOAD OCLRASTER_FUNC image_read_double_nearest(global const double2* img, const uint2 coord) {
 const uint2 img_size = oclr_get_image_size((image_header_ptr)img);
 return image_read_double_nearest(img, coord.y * img_size.x + coord.x);
}
double4 FUNC_OVERLOAD OCLRASTER_FUNC image_read_double_linear(global const double2* img, const float2 coord) {
 const uint2 img_size = oclr_get_image_size((image_header_ptr)img);
 global const double2* img_data_ptr = (global const double2*)((global const uchar*)img + OCLRASTER_IMAGE_HEADER_SIZE);
 const float2 fimg_size = convert_float2(img_size) - 1.0f;
 const float2 norm_coord = fmod(coord + fabs(floor(coord)), (float2)(1.0f, 1.0f));
...
void FUNC_OVERLOAD OCLRASTER_FUNC image_write_double(global double2* img, const uint2 coord, const double4 color) {
 const uint2 img_size = oclr_get_image_size((image_header_ptr)img);
 const uint offset = coord.y * img_size.x + coord.x;
...
double4 FUNC_OVERLOAD OCLRASTER_FUNC image_read_double_nearest(global const double3* img, const uint offset) {
...
double4 FUNC_OVERLOAD OCLRASTER_FUNC image_read_double_nearest(global const double3* img, const float2 coord) {
 const uint2 img_size = oclr_get_image_size((image_header_ptr)img);
 const float2 fimg_size = convert_float2(img_size) - 1.0f;
 const float2 norm_coord = fmod(coord + fabs(floor(coord)), (float2)(1.0f, 1.0f));
 const uint2 ui_tc = clamp(convert_uint2(norm_coord * fimg_size), (uint2)(0u, 0u), img_size - 1u);
...
double4 FUNC_OVERLOAD OCLRASTER_FUNC image_read_double_nearest(global const double3* img, const uint2 coord) {
 const uint2 img_size = oclr_get_image_size((image_header_ptr)img);
 return image_read_double_nearest(img, coord.y * img_size.x + coord.x);
}
double4 FUNC_OVERLOAD OCLRASTER_FUNC image_read_double_linear(global const double3* img, const float2 coord) {
 const uint2 img_size = oclr_get_image_size((image_header_ptr)img);
 global const double3* img_data_ptr = (global const double3*)((global const uchar*)img + OCLRASTER_IMAGE_HEADER_SIZE);
 const float2 fimg_size = convert_float2(img_size) - 1.0f;
 const float2 norm_coord = fmod(coord + fabs(floor(coord)), (float2)(1.0f, 1.0f));
...
void FUNC_OVERLOAD OCLRASTER_FUNC image_write_double(global double3* img, const uint2 coord, const double4 color) {
 const uint2 img_size = oclr_get_image_size((image_header_ptr)img);
 const uint offset = coord.y * img_size.x + coord.x;
...
double4 FUNC_OVERLOAD OCLRASTER_FUNC image_read_double_nearest(global const double4* img, const uint offset) {
...
double4 FUNC_OVERLOAD OCLRASTER_FUNC image_read_double_nearest(global const double4* img, const float2 coord) {
 const uint2 img_size = oclr_get_image_size((image_header_ptr)img);
 const float2 fimg_size = convert_float2(img_size) - 1.0f;
 const float2 norm_coord = fmod(coord + fabs(floor(coord)), (float2)(1.0f, 1.0f));
 const uint2 ui_tc = clamp(convert_uint2(norm_coord * fimg_size), (uint2)(0u, 0u), img_size - 1u);
...
double4 FUNC_OVERLOAD OCLRASTER_FUNC image_read_double_nearest(global const double4* img, const uint2 coord) {
 const uint2 img_size = oclr_get_image_size((image_header_ptr)img);
 return image_read_double_nearest(img, coord.y * img_size.x + coord.x);
}
double4 FUNC_OVERLOAD OCLRASTER_FUNC image_read_double_linear(global const double4* img, const float2 coord) {
 const uint2 img_size = oclr_get_image_size((image_header_ptr)img);
 global const double4* img_data_ptr = (global const double4*)((global const uchar*)img + OCLRASTER_IMAGE_HEADER_SIZE);
 const float2 fimg_size = convert_float2(img_size) - 1.0f;
 const float2 norm_coord = fmod(coord + fabs(floor(coord)), (float2)(1.0f, 1.0f));
...
void FUNC_OVERLOAD OCLRASTER_FUNC image_write_double(global double4* img, const uint2 coord, const double4 color) {
 const uint2 img_size = oclr_get_image_size((image_header_ptr)img);
 const uint offset = coord.y * img_size.x + coord.x;
