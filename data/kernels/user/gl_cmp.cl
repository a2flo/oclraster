...
float4 transform_main() {
	const float4 mv_vertex = mat4_mul_vec4(tp_uniforms->modelview_matrix,
										   (float4)(input_attributes->vertex.x,
													input_attributes->vertex.y,
													input_attributes->vertex.z,
													1.0f));
	output_attributes->vertex = mv_vertex;
	/*output_attributes->normal = mat4_mul_vec4(tp_uniforms->modelview_matrix,
...
bool rasterize_main() {
	...
	if(attenuation > 0.0f) {
		...
		const float lambert_term = dot((float3)(1.0f, 0.0f, 0.0f), light_dir);
		if(lambert_term > 0.0f) {
			...
			float3 R = reflect(-light_dir, (float3)(1.0f, 0.0f, 0.0f));
		...
		color.xyz *= image_read(diffuse_texture, linear_sampler, (float2)(0.5f, 0.5f)).xyz;
		framebuffer->color = color;
	}
#endif
	framebuffer->color = (float4)(barycentric, 1.0f);
	return true;
}
#endif
